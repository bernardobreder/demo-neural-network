...
public class SP500Actual {
	...
	private final Set<InterestRate> rates = new TreeSet<>();
	private final Set<FinancialSample> samples = new TreeSet<>();
	...
	public void getOutputData(final int offset, final double[] output) {
		...
		for (int i = 0; i < this.outputSize; i++) {
			final FinancialSample sample = (FinancialSample) samplesArray[offset + this.inputSize + i];
	...
	public void load(final String sp500Filename, final String primeFilename) throws IOException, ParseException {
		this.loadSP500(sp500Filename);
	...
	public void stitchInterestRates() {
		for (final FinancialSample sample : this.samples) {
			final double rate = this.getPrimeRate(sample.getDate());
			sample.setRate(rate);
		}
	}
