...
public class TestSOM extends JFrame implements Runnable {
	 ...
	 */
	TestSOM() {
		this.setTitle("Training a Self Organizing Map Neural Network");
		this.setSize(400, 450);
		final Toolkit toolkit = Toolkit.getDefaultToolkit();
		final Dimension d = toolkit.getScreenSize();
		this.setLocation((int) (d.width - this.getSize().getWidth()) / 2,
	...
	public void paint(Graphics g) {
		...
		if (this.offScreen == null) {
			this.offScreen = this.createImage((int) this.getBounds().getWidth(), (int) this.getBounds().getHeight());
		}
		g = this.offScreen.getGraphics();
		final int width = this.getContentPane().getWidth();
		final int height = this.getContentPane().getHeight();
		...
		g.drawString(
				"retry = " + this.retry + ",current error = " + nf.format(this.totalError * 100) + "%, best error = "
						+ nf.format(this.bestError * 100) + "%",
				0, (int) this.getContentPane().getBounds().getHeight());
		this.getContentPane().getGraphics().drawImage(this.offScreen, 0, 0, this);
