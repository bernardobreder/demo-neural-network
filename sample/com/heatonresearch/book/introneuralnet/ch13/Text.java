...
public class Text {
	 ...
	 */
	public static void checkURL(final ScanReportable report, final URL url, final Integer desiredYear)
			throws IOException {
		int ch;
		...
		do {
			...
			if ((ch != -1) && (ch != 0) && (ignoreUntil == null)) {
				...
				if (html.getTag().getName().equalsIgnoreCase("body") || html.getTag().getName().equalsIgnoreCase("br")
				...
				if (ignoreUntil == null) {
					if (html.getTag().getName().equalsIgnoreCase("script")) {
						ignoreUntil = "/script";
					} else if (html.getTag().getName().equalsIgnoreCase("style")) {
						ignoreUntil = "/style";
					}
				} else {
