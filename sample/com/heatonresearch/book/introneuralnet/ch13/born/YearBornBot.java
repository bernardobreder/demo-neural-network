...
public class YearBornBot implements ScanReportable {
	...
	public YearBornBot() throws IOException, ClassNotFoundException {
		this.network = (FeedforwardNetwork) SerializeObject.load(Config.FILENAME_WHENBORN_NET);
		this.histogram = (WordHistogram) SerializeObject.load(Config.FILENAME_HISTOGRAM);
	}
	...
	public void process(final String name) throws IOException {
		...
		final int resultYear = this.getResult();
		if (resultYear == -1) {
