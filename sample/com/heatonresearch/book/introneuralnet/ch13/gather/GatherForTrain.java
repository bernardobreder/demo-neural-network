...
public class GatherForTrain implements ScanReportable {
	...
	private final Collection<String> trainingDataGood = new Vector<>();
	private final Collection<String> trainingDataBad = new Vector<>();
	...
	public void process() throws IOException {
		this.pool = Executors.newFixedThreadPool(GatherForTrain.THREAD_POOL_SIZE);
		final Collection<Callable<Integer>> tasks = new ArrayList<>();
		final ReadCSV famous = new ReadCSV("famous.csv");
		this.report("Building training data from list of famous people.");
		...
		try {
			...
			this.report("Done collecting Internet data.");
		...
		length /= 1000L;
		length /= 60;
		System.out.println("Took " + length + " minutes to collect training data from the Internet.");
		System.out.println("Writing training file");
		this.writeTrainingFile();
	...
	public void reportDone(final String string) {
		this.report(this.currentTask + "/" + this.totalTasks + ":" + string);
		this.currentTask++;
	}
