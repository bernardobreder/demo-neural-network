...
public class AnalyzeSentences {
	...
	public void process(final TrainingSet set, final double value, final String filename) throws IOException {
		this.countLines(filename);
		this.processLines(filename, set, value);
	}

	private void processLine(final String line, final TrainingSet set, final double value) {
	...
	private void processLines(final String filename, final TrainingSet set, final double value) throws IOException {
		String line = null;
		final BufferedReader br = new BufferedReader(new FileReader(filename));
		while ((line = br.readLine()) != null) {
			this.processLine(line, set, value);
		}
		br.close();
	}
