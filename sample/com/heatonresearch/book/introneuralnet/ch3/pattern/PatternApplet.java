 ...
 *
 * PatternApplet: An applet that displays an 8X8 grid that presents patterns to
 * a Hopfield neural network.
 *
 * @author Jeff Heaton
 * @version 2.1
 */
public class PatternApplet extends Applet implements MouseListener, ActionListener {
	...
	public void paint(final Graphics g) {
		...
		for (int y = 0; y < this.GRID_Y; y++) {
			for (int x = 0; x < this.GRID_X; x++) {
				if (this.grid[index++]) {
					g.fillRect(this.margin + (x * this.CELL_WIDTH), y * this.CELL_HEIGHT, this.CELL_WIDTH,
							this.CELL_HEIGHT);
				} else {
					g.drawRect(this.margin + (x * this.CELL_WIDTH), y * this.CELL_HEIGHT, this.CELL_WIDTH,
							this.CELL_HEIGHT);
				}
			}
		}
