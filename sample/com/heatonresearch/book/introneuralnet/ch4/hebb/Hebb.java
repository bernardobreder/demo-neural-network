...
public class Hebb {
	...
	protected void presentPattern(final double i1, final double i2) {
		...
		result = this.recognize(i1, i2);
		System.out.print(" result=" + result);

		// adjust weight 1
		delta = this.trainingFunction(this.rate, i1, result);
		this.w1 += delta;
		System.out.print(",delta w1=" + delta);

		// adjust weight 2
		delta = this.trainingFunction(this.rate, i2, result);
		this.w2 += delta;
	 ...
	 */
	protected double trainingFunction(final double rate, final double input, final double output) {
