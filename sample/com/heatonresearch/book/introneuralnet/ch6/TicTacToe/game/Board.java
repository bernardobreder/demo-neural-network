 ...
 * <code>byte[][]</code> and worked on with static methods rather than being an
 * object with behaviour so that AI players can have many many copies of boards
 * in memory without making the memory requirements outrageous.
 *
...
public class Board {
	 ...
	 * @return <code>boolean</code>, <code>true</code> if the specified player
	 *         has won.
	 */
	...
	public static boolean placePiece(final byte[][] board, final Move m) {
		...
		if (board[m.x][m.y] != TicTacToe.EMPTY) {
			throw new IllegalStateException(
					"Tried to play a piece on " + "top of another at (" + m.x + ", " + m.y + ")");
		} else {
		...
		for (int y = 0; y < board.length; y++) {
			...
			for (int x = 0; x < board.length; x++) {
				if (board[x][y] == TicTacToe.EMPTY) {
					...
					throw new IllegalStateException("Square (" + x + ", " + y + ") is not empty, white or black!");
				}
			}
