...
public class PlayerLogic implements Player {
	...
	private Move canWin(final byte board[][], final byte color) {
		for (int i = 0; i < 3; i++) {
			...
			final Move move = this.findWin(board, move1, move2, move3, color);
			if (move != null) {
				return move;
			}
		}
		...
		Move move = this.findWin(board, move1, move2, move3, color);
		if (move != null) {
			return move;
		}
		...
		move = this.findWin(board, move1, move2, move3, color);
		if (move != null) {
			return move;
		}
	...
	private Move findWin(final byte[][] board, final Move move1, final Move move2, final Move move3, final int color) {
		if ((board[move1.x][move1.y] == color) && (board[move2.x][move2.y] == color)
				&& (board[move3.x][move3.y] == TicTacToe.EMPTY)) {
			return move3;
		} else if ((board[move2.x][move2.y] == color) && (board[move3.x][move3.y] == color)
				&& (board[move1.x][move1.y] == TicTacToe.EMPTY)) {
			return move1;
		} else if ((board[move1.x][move1.y] == color) && (board[move3.x][move3.y] == color)
	...
	public Move getMove(final byte[][] board, final Move prev, final byte color) {
		...
		if (result == null) {
			result = this.canWin(board, color);
		}

		if (result == null) {
			result = this.canWin(board, other);
		}

		// Step 1: Is the center open
		if (result == null) {
			result = this.centerOpen(board, color);
		}

		// Step 2: Is the corner open
		if (result == null) {
			result = this.cornerOpen(board, color);
		}

		// Step 3: Move somewhere
		if (result == null) {
			result = this.moveSomewhere(board, color);
		}
