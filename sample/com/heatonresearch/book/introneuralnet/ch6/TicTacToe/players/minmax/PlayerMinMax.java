 ...
 * <a href=
 * "http://mainline.brynmawr.edu/Courses/cs372/fall2000/AB.html">http://mainline.brynmawr.edu/Courses/cs372/fall2000/AB.html</a>.
 *
...
public class PlayerMinMax implements Player {
	...
	public Move getMove(final byte[][] board, final Move prev, final byte color) {
		...
		while ((child = root.getChild()) != null) {
			final byte val = this.minimaxAB(child, true, Byte.MIN_VALUE, Byte.MAX_VALUE);
			if (val >= max) {
				max = val;
				bestNode = child;
			}
	...
	private byte minimaxAB(final Node n, final boolean min, final byte a, final byte b) {
		...
		if (min) {
			while (((child = n.getChild()) != null) && !(alpha > beta)) {
				final byte val = this.minimaxAB(child, false, alpha, beta);
				if (val < beta) {
					beta = val;
				}
			}
			...
			while (((child = n.getChild()) != null) && !(alpha > beta)) {
				final byte val = this.minimaxAB(child, true, alpha, beta);
				if (val > alpha) {
					alpha = val;
				}
			}
