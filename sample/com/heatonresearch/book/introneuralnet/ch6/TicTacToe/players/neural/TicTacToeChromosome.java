 ...
 *
 * TicTacToeChromosome: Implements a chromosome for the neural network that
 * plays tic-tac-toe.
 *
 * @author Jeff Heaton
 * @version 2.1
...
public class TicTacToeChromosome extends NeuralChromosome<TicTacToeGenetic> {
	 ...
	 */
	public TicTacToeChromosome(final TicTacToeGenetic genetic, final FeedforwardNetwork network)
			throws NeuralNetworkError {
		this.setGeneticAlgorithm(genetic);
		this.setNetwork(network);

		this.initGenes(network.getWeightMatrixSize());
		this.updateGenes();
	}
	...
	public void calculateCost() {
		...
		try {
			...
			final PlayerNeural player1 = new PlayerNeural(this.getNetwork());
			Player player2;

			player2 = (Player) this.getGeneticAlgorithm().getOpponent().newInstance();
			final ScorePlayer score = new ScorePlayer(player1, player2, false);
			this.setCost(score.score());
