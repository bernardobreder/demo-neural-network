 ...
 *
 * TicTacToeGenetic: Use a genetic algorithm to teach a neural network to play
 * Tic-Tac-Toe. The cost of each chromosome is calculated by playing the neural
 * network against a computer player.
 *
 * @author Jeff Heaton
 * @version 2.1
 */
public class TicTacToeGenetic extends NeuralGeneticAlgorithm<TicTacToeGenetic> {
	...
	public TicTacToeGenetic(final FeedforwardNetwork network, final boolean reset, final int populationSize,
			final double mutationPercent, final double percentToMate, final Class<?> opponent)
		...
		this.setChromosomes(new TicTacToeChromosome[this.getPopulationSize()]);
	...
	public double getScore() {
		final NeuralChromosome<TicTacToeGenetic> c = this.getChromosome(0);
		return c.getCost();
	}
