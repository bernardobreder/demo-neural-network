 ...
 * @version 2.1
 */
public class GeneticTravelingSalesmanFrame extends JFrame implements Runnable, ComponentListener {
	...
	public static void main(final String args[]) {
		(new GeneticTravelingSalesmanFrame()).setVisible(true);
	}
	 ...
	 * The constructor
	 */
	public GeneticTravelingSalesmanFrame() {
	...
	public void componentShown(final ComponentEvent e) {
		...
		if (this.map == null) {
			...
			this.getContentPane().add(this.map, "Center");
			this.status = new JLabel("Starting up");
			this.getContentPane().add(this.status, "South");
		}

		// place the cities at random locations
		final int height = this.getBounds().height - 50;
		final int width = this.getBounds().width - 10;
		this.cities = new City[GeneticTravelingSalesmanFrame.CITY_COUNT];
		...
		this.genetic = new TSPGeneticAlgorithm(this.cities, GeneticTravelingSalesmanFrame.POPULATION_SIZE,
				GeneticTravelingSalesmanFrame.MUTATION_PERCENT, 0.25, 0.5,
				GeneticTravelingSalesmanFrame.CITY_COUNT / 5);
	...
	public void run() {
		...
		while (countSame < 100) {
			...
			this.status.setText(
					"Generation " + this.generation + " Cost " + (int) thisCost + " Mutated " + nf.format(0) + "%");
