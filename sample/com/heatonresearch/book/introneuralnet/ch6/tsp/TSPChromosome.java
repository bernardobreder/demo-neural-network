 ...
 *
 * TSPChromosome: A chromosome that is used to attempt to solve the traveling
 * salesman problem. A chromosome is a list of cities.
 *
 * @author Jeff Heaton
 * @version 2.1
 */
public class TSPChromosome extends Chromosome<Integer, TSPGeneticAlgorithm> {
	...
	public void calculateCost() throws NeuralNetworkError {
		...
		for (int i = 0; i < this.cities.length - 1; i++) {
			final double dist = this.cities[this.getGene(i)].proximity(this.cities[this.getGene(i + 1)]);
			cost += dist;
		}
		this.setCost(cost);
	...
	public void mutate() {
		...
		final Integer temp = this.getGene(iswap1);
		this.setGene(iswap1, this.getGene(iswap2));
		this.setGene(iswap2, temp);
	}
