...
public class GeneticXOR {
	...
	public static void main(final String args[]) {
		...
		final TrainingSetNeuralGeneticAlgorithm train = new TrainingSetNeuralGeneticAlgorithm(network, true, XOR_INPUT,
				XOR_IDEAL, 5000, 0.1, 0.25);

		int epoch = 1;

		do {
			train.iteration();
			System.out.println("Epoch #" + epoch + " Error:" + new BigDecimal(train.getError()));
			epoch++;
		...
		for (int i = 0; i < XOR_IDEAL.length; i++) {
			...
			System.out.println(
					XOR_INPUT[i][0] + "," + XOR_INPUT[i][1] + ", actual=" + actual[0] + ",ideal=" + XOR_IDEAL[i][0]);
		}
	}
}
