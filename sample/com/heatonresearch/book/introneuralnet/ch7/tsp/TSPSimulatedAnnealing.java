 ...
 *
 * TSPSimulatedAnnealing: Implementation of the simulated annealing algorithm
 * that trys to solve the traveling salesman problem.
 *
 * @author Jeff Heaton
 * @version 2.1
 */
public class TSPSimulatedAnnealing extends SimulatedAnnealing<Integer> {
	 ...
	 */
	public TSPSimulatedAnnealing(final City cities[], final double startTemp, final double stopTemp, final int cycles) {
	...
	public void randomize() {
		...
		for (int i = 0; i < this.temperature; i++) {
			...
			final double d = this.distance(index1, index1 + 1) + this.distance(index2, index2 + 1)
					- this.distance(index1, index2) - this.distance(index1 + 1, index2 + 1);
			if (d > 0) {
