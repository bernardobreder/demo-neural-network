 ...
 * @version 2.1
 */
public class PruneSelectiveApp extends JFrame implements ActionListener, Runnable {
	...
	public PruneSelectiveApp() {
		this.setTitle("XOR Solution - Select Prune");
	...
	protected void evaluate() {
		final double xorData[][] = this.getGrid();
	...
	public void prune() {
		final double xorData[][] = this.getGrid();
		final double xorIdeal[][] = this.getIdeal();
	...
	public void run() {
		...
		final Train train = new Backpropagation(this.network, xorData, xorIdeal, 0.7, 0.9);
		...
		for (int i = 0; i < max; i++) {
			...
			update++;
			if (update == 100) {
				this.status.setText("Cycles Left:" + (max - i) + ",Error:" + train.getError());
				update = 0;
			}
		}
