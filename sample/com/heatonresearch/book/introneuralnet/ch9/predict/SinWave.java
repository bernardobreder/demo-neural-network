...
public class SinWave {
	...
	public static int NEURONS_HIDDEN_1 = 7;

	public static int NEURONS_HIDDEN_2 = 0;
	...
	public void createNetwork() {
		...
		this.network.addLayer(new FeedforwardLayer(threshold, SinWave.NEURONS_HIDDEN_1));
		if (SinWave.NEURONS_HIDDEN_2 > 0) {
			this.network.addLayer(new FeedforwardLayer(threshold, SinWave.NEURONS_HIDDEN_2));
		}
	...
	private void display() {
		...
		double input[] = new double[SinWave.INPUT_SIZE];
		double output[] = new double[SinWave.OUTPUT_SIZE];
	...
	private void trainNetworkAnneal() {
		NeuralSimulatedAnnealing train = new NeuralSimulatedAnnealing(this.network, this.input, this.ideal, 10, 2, 100);
		int epoch = 1;
		do {
			train.iteration();
	...
	private void trainNetworkBackprop() {
		Train train = new Backpropagation(this.network, this.input, this.ideal, 0.001, 0.1);
		int epoch = 1;
		do {
			train.iteration();
			System.out.println("Iteration #" + epoch + " Error:" + train.getError());
