...
public class Backpropagation implements Train {
	 ...
	 */
	private Map<FeedforwardLayer, BackpropagationLayer> layerMap = new HashMap<>();
	 ...
	 */
	public Backpropagation(FeedforwardNetwork network, double input[][], double ideal[][], double learnRate,
			double momentum) {
	...
	public void calcError(double ideal[]) {
		if (ideal.length != this.network.getOutputLayer().getNeuronCount()) {
			throw new NeuralNetworkError("Size mismatch: Can't calcError for ideal input size=" + ideal.length
					+ " for output layer size=" + this.network.getOutputLayer().getNeuronCount());
		}
		// clear out all previous error data
		for (FeedforwardLayer layer : this.network.getLayers()) {
			this.getBackpropagationLayer(layer).clearError();
		}
		for (int i = this.network.getLayers().size() - 1; i >= 0; i--) {
			FeedforwardLayer layer = this.network.getLayers().get(i);
			if (layer.isOutput()) {
	...
	public BackpropagationLayer getBackpropagationLayer(FeedforwardLayer layer) {
		BackpropagationLayer result = this.layerMap.get(layer);
