...
public class BackpropagationLayer {
	...
	public void calcError() {
		...
		for (int i = 0; i < this.layer.getNext().getNeuronCount(); i++) {
			for (int j = 0; j < this.layer.getNeuronCount(); j++) {
				this.accumulateMatrixDelta(j, i, next.getErrorDelta(i) * this.layer.getFire(j));
				this.setError(j, this.getError(j) + this.layer.getMatrix().get(j, i) * next.getErrorDelta(i));
			}
	...
	public void calcError(final double ideal[]) {
		...
		for (int i = 0; i < this.layer.getNeuronCount(); i++) {
			this.setError(i, ideal[i] - this.layer.getFire(i));
			this.setErrorDelta(i, BoundNumbers.bound(this.calculateDelta(i)));
		}
	}
	...
	private double calculateDelta(final int i) {
		return this.getError(i) * this.layer.getActivationFunction().derivativeFunction(this.layer.getFire(i));
	}
