...
abstract public class Chromosome<GENE_TYPE, GA_TYPE extends GeneticAlgorithm<?>>
	...
	public void mate(final Chromosome<GENE_TYPE, GA_TYPE> father, final Chromosome<GENE_TYPE, GA_TYPE> offspring1,
			final Chromosome<GENE_TYPE, GA_TYPE> offspring2) throws NeuralNetworkError {
		final int geneLength = this.getGenes().length;

		// the chromosome must be cut at two positions, determine them
		final int cutpoint1 = (int) (Math.random() * (geneLength - this.getGeneticAlgorithm().getCutLength()));
		final int cutpoint2 = cutpoint1 + this.getGeneticAlgorithm().getCutLength();

		// keep track of which cities have been taken in each of the two
		// offspring, defaults to false.
		final Set<GENE_TYPE> taken1 = new HashSet<>();
		final Set<GENE_TYPE> taken2 = new HashSet<>();
		...
		for (int i = 0; i < geneLength; i++) {
			if ((i < cutpoint1) || (i > cutpoint2)) {
				if (this.getGeneticAlgorithm().isPreventRepeat()) {
					offspring1.setGene(i, this.getNotTaken(this, taken1));
					offspring2.setGene(i, this.getNotTaken(father, taken2));
				} else {
