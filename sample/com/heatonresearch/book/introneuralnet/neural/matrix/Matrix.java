...
public class Matrix implements Cloneable, Serializable {
	...
	public void add(final int row, final int col, final double value) {
		...
		final double newValue = this.get(row, col) + value;
	...
	public boolean equals(final Matrix matrix, int precision) {
		...
		for (int r = 0; r < this.getRows(); r++) {
			for (int c = 0; c < this.getCols(); c++) {
				if ((long) (this.get(r, c) * precision) != (long) (matrix.get(r, c) * precision)) {
					return false;
				}
			}
		}
	...
	public Matrix getCol(final int col) {
		...
		final double newMatrix[][] = new double[this.getRows()][1];
	...
	public Matrix getRow(final int row) {
		...
		final double newMatrix[][] = new double[1][this.getCols()];
	...
	public Double[] toPackedArray() {
		final Double result[] = new Double[this.getRows() * this.getCols()];
