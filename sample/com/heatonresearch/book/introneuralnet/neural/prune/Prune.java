...
public class Prune {
	 ...
	 */
	public Prune(final double rate, final double momentum, final double train[][], final double ideal[][],
	 ...
	 * @param ideal
	 */
	public Prune(final FeedforwardNetwork network, final double train[][], final double ideal[][],
			final double maxError) {
	...
	protected FeedforwardNetwork clipHiddenNeuron(final int neuron) {
		final FeedforwardNetwork result = (FeedforwardNetwork) this.currentNetwork.clone();
	...
	protected boolean findNeuron() {
		...
		for (int i = 0; i < this.currentNetwork.getHiddenLayerCount(); i++) {
			...
			final double e2 = this.determineError(trial);
			if (e2 < this.maxError) {
	...
	protected void increment() {
		...
		if (doit) {
			...
			this.currentNetwork.addLayer(new FeedforwardLayer(this.ideal[0].length));
