...
public class TrainSelfOrganizingMap {
	 ...
	 *            The learning rate.
	 */
	public TrainSelfOrganizingMap(SelfOrganizingMap som, double train[][], LearningMethod learnMethod,
			double learnRate) {
		this.som = som;
		this.train = train;
		this.totalError = 1.0;
		...
		for (int tset = 0; tset < train.length; tset++) {
			Matrix dptr = Matrix.createColumnMatrix(train[tset]);
		...
		this.bestnet = new SelfOrganizingMap(this.inputNeuronCount, this.outputNeuronCount,
	...
	public void evaluateErrors() {
		...
		for (int tset = 0; tset < this.train.length; tset++) {
			double[] item = this.train[tset];
			NormalizeInput input = new NormalizeInput(item, this.som.getNormalizationType());
			...
			for (int i = 0; i < this.inputNeuronCount; i++) {
				diff = item[i] * input.getNormfac() - wptr.get(0, i);
				length += diff * diff;
				if (this.learnMethod == LearningMethod.SUBTRACTIVE) {
					this.correc.add(best, i, diff);
				} else {
					this.work.set(0, i, this.learnRate * item[i] * input.getNormfac() + wptr.get(0, i));
				}
			}
