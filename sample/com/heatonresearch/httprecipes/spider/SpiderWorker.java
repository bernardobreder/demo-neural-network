...
public class SpiderWorker implements Runnable {
	 ...
	 * The logger.
	 */
	private static Logger logger = Logger.getLogger("com.heatonresearch.httprecipes.spider.SpiderWorker");
	...
	public void run() {
		...
		try {
			...
			if (this.spider.getOptions().userAgent != null) {
				connection.setRequestProperty("User-Agent", this.spider.getOptions().userAgent);
			}
			...
			if (connection.getContentType().equalsIgnoreCase("text/html")) {
				SpiderParseHTML parse = new SpiderParseHTML(connection.getURL(), new SpiderInputStream(is, null),
						this.spider);
