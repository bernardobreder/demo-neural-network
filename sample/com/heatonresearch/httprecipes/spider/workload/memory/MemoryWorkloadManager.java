...
public class MemoryWorkloadManager implements WorkloadManager {
	 ...
	 */
	private Map<URL, URLStatus> workload = new HashMap<>();

	/**
	 * The list of those items, which are already in the workload, that are
	 * waiting for processing.
	 */
	private BlockingQueue<URL> waiting = new LinkedBlockingQueue<>();
	...
	public boolean add(URL url, URL source, int depth) {
		if (!this.contains(url)) {
			...
			this.setStatus(url, source, URLStatus.Status.WAITING, depth);
	...
	public URL getWork() {
		...
		try {
			...
			if (url != null) {
				this.setStatus(url, null, URLStatus.Status.WORKING, -1);
				this.workingCount++;
			}
			return url;
	...
	public void markError(URL url) {
		...
		this.setStatus(url, null, URLStatus.Status.ERROR, -1);
	...
	public void markProcessed(URL url) {
		...
		assert this.workingCount > 0;
		this.waiting.remove(url);
		this.setStatus(url, null, URLStatus.Status.PROCESSED, -1);
	}
	...
	public void resume() throws WorkloadException {
		throw (new WorkloadException("Memory based workload managers can not resume."));
	}
