...
public class ConsoleAnalystListener implements AnalystListener {
	 ...
	 */
	@Override
	public void reportCommandBegin(final int total, final int current, final String name) {
		System.out.println();
		if (total == 0) {
	...
	public void reportCommandEnd(final boolean cancel) {
		...
		System.out.println("Task " + this.currentTask + " " + cancelStr + ", task elapsed time "
				+ Format.formatTimeSpan((int) (this.stopwatch.getElapsedMilliseconds() / Format.MILI_IN_SEC)));
	...
	public void reportTraining(final MLTrain train) {
		...
		System.out.println("Iteration #" + Format.formatInteger(train.getIteration()) + " Error:"
				+ Format.formatPercent(train.getError()) + " elapsed time = "
				+ Format.formatTimeSpan((int) (this.stopwatch.getElapsedMilliseconds() / Format.MILI_IN_SEC)));
	}
