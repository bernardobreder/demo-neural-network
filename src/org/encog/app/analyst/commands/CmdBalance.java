...
public class CmdBalance extends Cmd {
	...
	public boolean executeCommand(final String args) {
		// get filenames
		final String sourceID = this.getProp().getPropertyString(ScriptProperties.BALANCE_CONFIG_SOURCE_FILE);
		final String targetID = this.getProp().getPropertyString(ScriptProperties.BALANCE_CONFIG_TARGET_FILE);
		...
		final int countPer = this.getProp().getPropertyInt(ScriptProperties.BALANCE_CONFIG_COUNT_PER);
		final String targetFieldStr = this.getProp().getPropertyString(ScriptProperties.BALANCE_CONFIG_BALANCE_FIELD);
		final DataField targetFieldDF = this.getAnalyst().getScript().findDataField(targetFieldStr);
		...
		final int targetFieldIndex = this.getAnalyst().getScript().findDataFieldIndex(targetFieldDF);
