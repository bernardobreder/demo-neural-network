...
public class CmdCode extends Cmd {
	...
	public boolean executeCommand(final String args) {
		// get filenames
		final String targetID = this.getProp().getPropertyString(ScriptProperties.CODE_CONFIG_TARGET_FILE);
		final File targetFile = this.getScript().resolveFilename(targetID);

		// get other options
		final TargetLanguage targetLanguage = this.getProp()
				.getPropertyTargetLanguage(ScriptProperties.CODE_CONFIG_TARGET_LANGUAGE);
		final boolean embedData = this.getProp().getPropertyBoolean(ScriptProperties.CODE_CONFIG_EMBED_DATA);
