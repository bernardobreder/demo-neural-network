...
public class CmdCreate extends Cmd {
	...
	public boolean executeCommand(final String args) {
		// get filenames
		final String trainingID = this.getProp().getPropertyString(ScriptProperties.ML_CONFIG_TRAINING_FILE);
		final String resourceID = this.getProp().getPropertyString(ScriptProperties.ML_CONFIG_MACHINE_LEARNING_FILE);

		final File trainingFile = this.getScript().resolveFilename(trainingID);
		...
		final String arch = this.getProp().getPropertyString(ScriptProperties.ML_CONFIG_ARCHITECTURE);

		EncogLogging.log(EncogLogging.LEVEL_DEBUG, "Beginning create");
		EncogLogging.log(EncogLogging.LEVEL_DEBUG, "training file:" + trainingID);
	...
	private void handlePrgPopulation(PrgPopulation pop) {
		...
		for (AnalystField field : this.getScript().getNormalize().getNormalizedFields()) {
			...
			case SingleField:
				if (df.isClass()) {
					mapping = new VariableMapping(varName, ValueType.enumType, classType++,
							df.getClassMembers().size());
