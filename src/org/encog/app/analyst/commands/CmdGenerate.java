...
public class CmdGenerate extends Cmd {
	...
	private int[] determineIdealFields(final CSVHeaders headerList) {
		...
		int[] result;
		final String type = this.getProp().getPropertyString(ScriptProperties.ML_CONFIG_TYPE);
		...
		for (int currentIndex = 0; currentIndex < headerList.size(); currentIndex++) {
			...
			final AnalystField field = this.getAnalyst().getScript().findNormalizedField(baseName, slice);
	...
	public boolean executeCommand(final String args) {
		// get filenames
		final String sourceID = this.getProp().getPropertyString(ScriptProperties.GENERATE_CONFIG_SOURCE_FILE);
		final String targetID = this.getProp().getPropertyString(ScriptProperties.GENERATE_CONFIG_TARGET_FILE);
		final CSVFormat format = this.getAnalyst().getScript().determineFormat();
		...
		final int[] input = this.determineInputFields(headerList);
		final int[] ideal = this.determineIdealFields(headerList);
