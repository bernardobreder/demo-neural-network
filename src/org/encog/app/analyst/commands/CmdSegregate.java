...
public class CmdSegregate extends Cmd {
	...
	public boolean executeCommand(final String args) {
		// get filenames
		final String sourceID = this.getProp().getPropertyString(ScriptProperties.SEGREGATE_CONFIG_SOURCE_FILE);
		...
		for (final AnalystSegregateTarget target : this.getScript().getSegregate().getSegregateTargets()) {
			final File filename = this.getScript().resolveFilename(target.getFile());
			...
			EncogLogging.log(EncogLogging.LEVEL_DEBUG,
					"target file:" + target.getFile() + ", Percent: " + Format.formatPercent(target.getPercent()));
