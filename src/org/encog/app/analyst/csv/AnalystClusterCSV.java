...
public class AnalystClusterCSV extends BasicFile {
	...
	public void analyze(final EncogAnalyst theAnalyst, final File inputFile, final boolean headers,
		...
		while (csv.next() && !this.shouldStop()) {
			...
			final double[] inputArray = AnalystNormalizeCSV.extractFields(this.analyst, this.analystHeaders, csv,
					outputLength, true);
	 ...
	 */
	private PrintWriter prepareOutputFile(final File outputFile, final int input, final int output) {
		try {
	 ...
	 */
	public void process(final File outputFile, final int clusters, final EncogAnalyst theAnalyst,
