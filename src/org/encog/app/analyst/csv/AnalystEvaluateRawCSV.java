...
public class AnalystEvaluateRawCSV extends BasicFile {
	 ...
	 */
	public void analyze(final EncogAnalyst theAnalyst, final File inputFile, final boolean headers,
		...
		if ((this.getInputHeadings().length != this.inputCount)
				&& (this.getInputHeadings().length != (this.inputCount + this.outputCount))) {
			throw new AnalystError("Invalid number of columns(" + this.getInputHeadings().length
					+ "), must match input(" + this.inputCount + ") count or input+output("
	 ...
	 *            The method to use.
	 */
	public void process(final File outputFile, final MLRegression method) {
		...
		if (method.getInputCount() != this.inputCount) {
			throw new AnalystError("This machine learning method has " + method.getInputCount()
					+ " inputs, however, the data has " + this.inputCount + " inputs.");
		}

		MLData output = null;
		final MLData input = new BasicMLData(method.getInputCount());

		final PrintWriter tw = this.analystPrepareOutputFile(outputFile);
		...
		while (csv.next()) {
			...
			for (int i = 0; i < this.inputCount; i++) {
				...
				final double d = this.getFormat().parse(str);
				input.setData(i, d);
				dataIndex++;
			}
