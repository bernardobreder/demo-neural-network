...
public class TimeSeriesUtil {
	 ...
	 * The heading map.
	 */
	private final Map<String, Integer> headingMap = new HashMap<>();
	 ...
	 *            The analyst to use.
	 * @param includeOutput
	 *            whether the output columns will be included in input
	 * @param headings
	 *            The column headings.
	 */
	public TimeSeriesUtil(final EncogAnalyst theAnalyst, boolean includeOutput, final List<String> headings) {
		...
		this.inputSize = includeOutput ? this.analyst.determineTotalColumns()
				: this.analyst.determineTotalInputFieldCount();
	...
	public double[] process(final double[] input) {
		...
		for (final AnalystField field : this.analyst.getScript().getNormalize().getNormalizedFields()) {
			if (!field.isIgnored()) {
				...
				final int timeslice = this.translateTimeSlice(field.getTimeSlice());
	...
	/**
	 * Translate a timeslice from a pos/neg number to a displacement into the
	 * buffer.
	 *
	 * @param index
	 *            The index.
