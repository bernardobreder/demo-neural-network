...
public class ProcessExtension {
	...
	// add field
	public static final ProgramExtensionTemplate OPCODE_FIELD = new BasicTemplate(ProgramExtensionTemplate.NO_PREC,
			"field({s}{i}):{s}", NodeType.Function, true, 0) {
		/**
		 * The serial id.
		 */
		...
		public ExpressionValue evaluate(ProgramNode actual) {
			ProcessExtension pe = (ProcessExtension) actual.getOwner().getExtraData(EXTENSION_DATA_NAME);
	...
	// add fieldmax
	public static final ProgramExtensionTemplate OPCODE_FIELDMAX = new BasicTemplate(ProgramExtensionTemplate.NO_PREC,
			"fieldmax({s}{i}{i}):{f}", NodeType.Function, true, 0) {
		/**
		 * The serial id.
		 */
		private static final long serialVersionUID = 1L;
	...
	public static final ProgramExtensionTemplate OPCODE_FIELDMAXPIP = new BasicTemplate(
			ProgramExtensionTemplate.NO_PREC, "fieldmaxpip({s}{i}{i}):{f}", NodeType.Function, true, 0) {
		/**
		 * The serial id.
		 */
		private static final long serialVersionUID = 1L;
	...
	public String getField(String fieldName, int fieldIndex) {
		...
		if (fieldIndex >= this.data.size() || fieldIndex < 0) {
			throw new AnalystError("The specified temporal index " + fieldIndex
