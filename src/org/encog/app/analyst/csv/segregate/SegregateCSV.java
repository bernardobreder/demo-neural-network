...
public class SegregateCSV extends BasicFile {
	 ...
	 */
	private List<SegregateTargetPercent> targets = new ArrayList<>();
	 ...
	 */
	public void analyze(final File inputFile, final boolean headers, final CSVFormat format) {
	...
	private void balanceTargets() {
		...
		for (final SegregateTargetPercent p : this.targets) {
			...
			final int c = (int) (this.getRecordCount() * percent);
		...
		final int remain = this.getRecordCount() - numberAssigned;
	...
	public void process() {
		...
		for (final SegregateTargetPercent target : this.targets) {
			final PrintWriter tw = this.prepareOutputFile(target.getFilename());
