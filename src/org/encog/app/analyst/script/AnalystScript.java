...
public class AnalystScript {
	...
	public AnalystScript() {
		...
		this.properties.setProperty(ScriptProperties.SETUP_CONFIG_MAX_CLASS_COUNT, DEFAULT_MAX_CLASS);
	...
	public CSVFormat determineFormat() {
		return this.getProperties().getPropertyCSVFormat(ScriptProperties.SETUP_CONFIG_CSV_FORMAT);
	}
	...
	public AnalystField findNormalizedField(final String name, final int slice) {
		for (final AnalystField field : this.getNormalize().getNormalizedFields()) {
	...
	public File resolveFilename(final String sourceID) {
		final String name = this.getProperties().getFilename(sourceID);
