...
public class ScriptLoad {
	...
	private void handleDataClasses(final EncogFileSection section) {
		...
		final Map<String, List<AnalystClassItem>> map = new HashMap<>();
		...
		for (final DataField field : this.script.getFields()) {
			if (field.isClass()) {
				final List<AnalystClassItem> classList = map.get(field.getName());
				if (classList != null) {
	...
	private void handleDataStats(final EncogFileSection section) {
		final List<DataField> dfs = new ArrayList<>();
		boolean first = true;
		for (final String line : section.getLines()) {
			if (!first) {
				final List<String> cols = EncogFileSection.splitColumns(line);
	...
	private void handleSegregateFiles(final EncogFileSection section) {
		final List<AnalystSegregateTarget> nfs = new ArrayList<>();
		boolean first = true;
		for (final String line : section.getLines()) {
			if (!first) {
				final List<String> cols = EncogFileSection.splitColumns(line);
				...
				final AnalystSegregateTarget nf = new AnalystSegregateTarget(filename, percent);
				nfs.add(nf);
			} else {
				first = false;
			}
		...
		final AnalystSegregateTarget[] array = new AnalystSegregateTarget[nfs.size()];
		for (int i = 0; i < array.length; i++) {
			array[i] = nfs.get(i);
		}
	...
	private void loadSubSection(final EncogFileSection section) {
		...
		for (final String name : prop.keySet()) {
			final String key = section.getSectionName().toUpperCase() + ":" + section.getSubSectionName().toUpperCase()
					+ "_" + name;
	...
	private void processSubSection(final EncogFileSection section) {
		...
		if (currentSection.equals("SETUP") && currentSubsection.equalsIgnoreCase("CONFIG")) {
			...
			this.loadSubSection(section);
		} else if (currentSection.equals("ML") && currentSubsection.equalsIgnoreCase("OPCODES")) {
			this.loadOpcodes(section);
			...
			this.loadSubSection(section);
		} else if (currentSection.equals("CODE") && currentSubsection.equalsIgnoreCase("CONFIG")) {
			this.loadSubSection(section);
		} else if (currentSection.equals("PROCESS") && currentSubsection.equalsIgnoreCase("CONFIG")) {
			this.loadSubSection(section);
		} else if (currentSection.equals("PROCESS") && currentSubsection.equalsIgnoreCase("FIELDS")) {
			this.handleProcessFields(section);
		}
	}
	 ...
	 */
	private void validateProperty(final String section, final String subSection, final String name,
			final String value) {
		final PropertyEntry entry = PropertyConstraints.getInstance().getEntry(section, subSection, name);
		if (entry == null) {
