...
public class AnalystField {
	 ...
	 * The list of classes.
	 */
	private final List<ClassItem> classes = new ArrayList<>();
	 ...
	 *            The normalized low.
	 */
	public AnalystField(final double theNormalizedHigh, final double theNormalizedLow) {
	 ...
	 *            The name of this column.
	 */
	public AnalystField(final NormalizationAction theAction, final String theName) {
	 ...
	 *            The normalized low.
	 */
	public AnalystField(final NormalizationAction theAction, final String theName, final double ahigh,
			final double alow, final double nhigh, final double nlow) {
	 ...
	 *            The low value.
	 */
	public AnalystField(final String theName, final NormalizationAction theAction, final double high,
			final double low) {
		this.name = theName;
		this.action = theAction;
	 ...
	 *
	 * @param line
	 *            The line to write the raw headings to.
	 * @param prefix
	 *            The prefix to place.
	 * @param format
	 *            The format to use.
	 */
	public void addRawHeadings(final StringBuilder line, final String prefix, final CSVFormat format) {
		final int subFields = this.getColumnsNeeded();
	...
	public double deNormalize(final double value) {
		final double result = ((this.actualLow - this.actualHigh) * value - this.normalizedHigh * this.actualLow
				+ this.actualHigh * this.normalizedLow) / (this.normalizedLow - this.normalizedHigh);
	...
	public ClassItem determineClass(final int pos, final double[] data) {
		int resultIndex = 0;
		final double[] d = new double[this.getColumnsNeeded()];
	...
	private double[] encodeOneOf(final int classNumber) {
		final double[] result = new double[this.getColumnsNeeded()];
	...
	public boolean isClassify() {
		return (this.action == NormalizationAction.Equilateral) || (this.action == NormalizationAction.OneOf)
	 ...
	 *            The low value.
	 */
	public void makeClass(final NormalizationAction theAction, final int classFrom, final int classTo, final int high,
			final int low) {

		if ((this.action != NormalizationAction.Equilateral) && (this.action != NormalizationAction.OneOf)
				&& (this.action != NormalizationAction.SingleField)) {
	 ...
	 *            The low value.
	 */
	public void makeClass(final NormalizationAction theAction, final String[] cls, final double high,
			final double low) {
		if ((this.action != NormalizationAction.Equilateral) && (this.action != NormalizationAction.OneOf)
				&& (this.action != NormalizationAction.SingleField)) {
			throw new QuantError("Unsupported normalization type");
