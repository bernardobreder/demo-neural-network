...
public class AnalystNormalize {
	...
	/**
	 * The normalized fields. These fields define the order and format that data
	 * will be presented to the ML method.
	 */
	...
	public void init(final AnalystScript script) {
		...
		for (final AnalystField norm : this.normalizedFields) {
			...
			if ((norm.getAction() == NormalizationAction.Equilateral) || (norm.getAction() == NormalizationAction.OneOf)
	...
	public HandleMissingValues getMissingValues() {
		final String type = this.script.getProperties().getPropertyString(ScriptProperties.ML_CONFIG_TYPE);
	...
	public void setMissingValues(HandleMissingValues missingValues) {
		this.script.getProperties().setProperty(ScriptProperties.NORMALIZE_MISSING_VALUES,
				missingValues.getClass().getSimpleName());
	}
}
