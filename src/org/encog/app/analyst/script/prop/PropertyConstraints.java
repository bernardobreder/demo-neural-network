 ...
 * might have. This constant information allows values to be validated. This
...
public final class PropertyConstraints {
	...
	private PropertyConstraints() {
		...
		try {
			...
			final InputStream is = ResourceInputStream.openResourceInputStream("org/encog/data/analyst.csv");
			...
			while (csv.next()) {
				...
				final PropertyEntry entry = new PropertyEntry(t, nameStr, sectionStr);
	 ...
	 */
	public List<PropertyEntry> getEntries(final String section, final String subSection) {
		final String key = section + ":" + subSection;
		return this.data.get(key);
	}

	/**
	 * Get a single property entry. If the section and subsection do not exist,
	 * an error is thrown.
	 *
	 * @param section
	 *            The section.
	 ...
	 */
	public PropertyEntry getEntry(final String section, final String subSection, final String name) {
		final String key = section.toUpperCase() + ":" + subSection.toUpperCase();
