...
public class AnalystUtility {
	 ...
	 *
	 * @param includeInput
	 *            Should we include the input fields.
	 * @param includeOutput
	 *            Should we include the output fields.
	 * @param rawData
	 *            The raw data to encode from.
	 * @param encodedData
	...
	public MLDataSet loadCSV(File file) {
		...
		try {
			...
			while (csv.next()) {
				...
				double[] output = AnalystNormalizeCSV.extractFields(this.analyst, analystHeaders, csv, totalCount,
						false);
	...
	public void analyze(File theFilename) {
		final boolean headers = this.analyst.getScript().getProperties()
				.getPropertyBoolean(ScriptProperties.SETUP_CONFIG_INPUT_HEADERS);
		final AnalystFileFormat fmt = this.analyst.getScript().getProperties()
				.getPropertyFormat(ScriptProperties.SETUP_CONFIG_CSV_FORMAT);
