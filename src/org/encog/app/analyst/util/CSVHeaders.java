...
public class CSVHeaders {
	 ...
	 */
	public static String tagColumn(final String name, final int part, final int timeSlice, final boolean multiPart) {
	 ...
	 *            The filename.
	 * @param headers
	 *            False if headers are not extended.
	 * @param format
	 *            The CSV format.
	 */
	...
	private void validateSameName() {
		for (int i = 0; i < this.headerList.size(); i++) {
			for (int j = 0; j < this.headerList.size(); j++) {
				...
				if (this.headerList.get(i).equalsIgnoreCase(this.headerList.get(j))) {
