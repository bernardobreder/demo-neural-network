...
public class AnalystWizard {
	...
	private void determineTargetField() {
		final List<AnalystField> fields = this.script.getNormalize().getNormalizedFields();
		...
		if (this.targetFieldName == null || this.targetFieldName.length() == 0) {
			...
			if (this.goal == AnalystGoal.Classification) {
				...
				for (final AnalystField field : fields) {
					final DataField df = this.script.findDataField(field.getName());
			...
			if (!success) {
				throw new AnalystError("Can't determine target field automatically, "
		...
		if (!this.timeSeries && this.taskBalance) {
			this.script.getProperties().setProperty(ScriptProperties.BALANCE_CONFIG_BALANCE_FIELD,
					this.targetField.getName());
			final DataField field = this.analyst.getScript().findDataField(this.targetField.getName());
			if ((field != null) && field.isClass()) {
				final int countPer = field.getMinClassCount();
				this.script.getProperties().setProperty(ScriptProperties.BALANCE_CONFIG_COUNT_PER, countPer);
			} else {
				throw new AnalystError(
						"Balance currently may only be used on a nominal/class field, not a numeric field.");
			}
		}
		...
		if (this.methodType != WizardMethodType.BayesianNetwork) {
			...
			for (final AnalystField field : this.analyst.getScript().getNormalize().getNormalizedFields()) {
				if ((field.getAction() != NormalizationAction.Ignore) && field == this.targetField) {
			...
			if (af != null) {
				af.setOutput(true);
				/*
				 * However, if we are including the target field in the input,
				 * for time series, then we must create an input field for it.
				 */
		...
		if (this.taskCluster) {
			if ((this.targetField == null) || (this.goal != AnalystGoal.Classification)) {
				...
				final DataField tf = this.script.findDataField(this.targetField.getName());
	...
	private void expandTimeSlices() {
		final List<AnalystField> oldList = this.script.getNormalize().getNormalizedFields();
		final List<AnalystField> newList = new ArrayList<>();
	 ...
	 */
	private void generateFeedForward(final int inputColumns, final int outputColumns) {
		int hidden = (int) ((inputColumns * Math.max(this.lagWindowSize, 1)) * 1.5);
		if (hidden < 0) {
			// Usual cause of this is zero inputs, which is in turn caused by
			// not including the predicted field in the input.
		...
		if (this.range == NormalizeRange.NegOne2One) {
			this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_ARCHITECTURE,
	 ...
	 *            The output column count.
	 */
	private void generateNEAT(final int inputColumns, final int outputColumns) {
	 ...
	 *            The output column count.
	 */
	private void generateEPL(final int inputColumns, final int outputColumns) {
	 ...
	 *            The output column count.
	 */
	private void generateBayesian(final int inputColumns, final int outputColumns) {
		...
		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_TYPE, MLMethodFactory.TYPE_BAYESIAN);

		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_ARCHITECTURE, a.toString());
	...
	private void generateFilenames(final File rawFile) {
		...
		if (this.preprocess) {
			this.filenameProcess = FileUtil.addFilenameBase(rawFile, "_process").getName();
		}
	...
	private void generateGenerate() {
		...
		final int inputColumns = this.script.getNormalize().calculateInputColumns();
		final int idealColumns = this.script.getNormalize().calculateOutputColumns();

		switch (this.methodType) {
		case BayesianNetwork:
			this.generateBayesian(inputColumns, idealColumns);
			break;
		case FeedForward:
		...
		case SOM:
			...
			break;
		case PNN:
			this.generatePNN(inputColumns, idealColumns);
			break;
		case NEAT:
			this.generateNEAT(inputColumns, idealColumns);
			break;
		case EPL:
			this.generateEPL(inputColumns, idealColumns);
			break;
		default:
	...
	private void generateNormalizedFields() {
		final List<AnalystField> norm = this.script.getNormalize().getNormalizedFields();
		norm.clear();
	...
	private void generateSegregate() {
		if (this.taskSegregate) {
			...
			array[0] = new AnalystSegregateTarget(AnalystWizard.FILE_TRAIN, DEFAULT_TRAIN_PERCENT);
			array[1] = new AnalystSegregateTarget(AnalystWizard.FILE_EVAL, DEFAULT_EVAL_PERCENT);
	...
	private void generateSettings() {
		...
		this.script.getProperties().setProperty(ScriptProperties.HEADER_DATASOURCE_RAW_FILE, target);

		// preprocess
		if (this.preprocess) {
			this.script.getProperties().setProperty(ScriptProperties.PROCESS_CONFIG_SOURCE_FILE, target);
			target = AnalystWizard.FILE_PRE;
			this.script.getProperties().setProperty(ScriptProperties.PROCESS_CONFIG_TARGET_FILE, target);
		}

		// randomize
		if (!this.timeSeries && this.taskRandomize) {
			this.script.getProperties().setProperty(ScriptProperties.RANDOMIZE_CONFIG_SOURCE_FILE, target);
			target = AnalystWizard.FILE_RANDOM;
			this.script.getProperties().setProperty(ScriptProperties.RANDOMIZE_CONFIG_TARGET_FILE, target);
		}

		// balance
		if (!this.timeSeries && this.taskBalance) {
			this.script.getProperties().setProperty(ScriptProperties.BALANCE_CONFIG_SOURCE_FILE, target);
			target = AnalystWizard.FILE_BALANCE;
			this.script.getProperties().setProperty(ScriptProperties.BALANCE_CONFIG_TARGET_FILE, target);
		}

		// segregate
		if (this.taskSegregate) {
			this.script.getProperties().setProperty(ScriptProperties.SEGREGATE_CONFIG_SOURCE_FILE, target);
			target = AnalystWizard.FILE_TRAIN;
		}

		// normalize
		if (this.taskNormalize) {
			this.script.getProperties().setProperty(ScriptProperties.NORMALIZE_CONFIG_SOURCE_FILE, target);
			target = AnalystWizard.FILE_NORMALIZE;
			this.script.getProperties().setProperty(ScriptProperties.NORMALIZE_CONFIG_TARGET_FILE, target);
		...
		// cluster
		if (this.taskCluster) {
			this.script.getProperties().setProperty(ScriptProperties.CLUSTER_CONFIG_SOURCE_FILE, evalSource);
			this.script.getProperties().setProperty(ScriptProperties.CLUSTER_CONFIG_TARGET_FILE,
					AnalystWizard.FILE_CLUSTER);
			this.script.getProperties().setProperty(ScriptProperties.CLUSTER_CONFIG_TYPE, "kmeans");
		}

		// generate
		this.script.getProperties().setProperty(ScriptProperties.GENERATE_CONFIG_SOURCE_FILE, target);
		this.script.getProperties().setProperty(ScriptProperties.GENERATE_CONFIG_TARGET_FILE,
				AnalystWizard.FILE_TRAINSET);

		// ML
		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_TRAINING_FILE, AnalystWizard.FILE_TRAINSET);
		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_MACHINE_LEARNING_FILE,
				AnalystWizard.FILE_ML);
		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_OUTPUT_FILE, AnalystWizard.FILE_OUTPUT);

		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_EVAL_FILE, evalSource);
	...
	private void generateSVM(final int inputColumns, final int outputColumns) {
		...
		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_ARCHITECTURE, arch.toString());
	...
	private void generatePNN(final int inputColumns, final int outputColumns) {
		...
		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_TYPE, MLMethodFactory.TYPE_PNN);
		this.script.getProperties().setProperty(ScriptProperties.ML_CONFIG_ARCHITECTURE, arch.toString());

		this.script.getProperties().setProperty(ScriptProperties.ML_TRAIN_TYPE, MLTrainFactory.TYPE_PNN);
		this.script.getProperties().setProperty(ScriptProperties.ML_TRAIN_TARGET_ERROR, this.maxError);
	}
	...
	private void generateTasks() {
		...
		task3.getLines().add(this.createSet(ScriptProperties.ML_CONFIG_EVAL_FILE, AnalystWizard.FILE_EVAL_NORM));
		task3.getLines().add(this.createSet(ScriptProperties.NORMALIZE_CONFIG_SOURCE_FILE, AnalystWizard.FILE_EVAL));
		task3.getLines()
				.add(this.createSet(ScriptProperties.NORMALIZE_CONFIG_TARGET_FILE, AnalystWizard.FILE_EVAL_NORM));
	...
	public void reanalyze() {
		final String rawID = this.script.getProperties().getPropertyFile(ScriptProperties.HEADER_DATASOURCE_RAW_FILE);

		final File rawFilename = this.analyst.getScript().resolveFilename(rawID);

		this.analyst.reanalyze(rawFilename,
				this.script.getProperties().getPropertyBoolean(ScriptProperties.SETUP_CONFIG_INPUT_HEADERS),
				this.script.getProperties().getPropertyFormat(ScriptProperties.SETUP_CONFIG_CSV_FORMAT));
	...
	public void wizardRealTime(List<SourceElement> sourceData, File csvFile, int backwardWindow, int forwardWindow,
		...
		this.script.getProperties().setProperty(ScriptProperties.HEADER_DATASOURCE_RAW_FILE, csvFile);
		...
		this.setMissing(new DiscardMissing());

		this.setGoal(AnalystGoal.Regression);
		this.setRange(NormalizeRange.NegOne2One);
		this.setTaskNormalize(true);
		this.setTaskRandomize(false);
		this.setTaskSegregate(true);
		this.setTaskBalance(false);
		this.setTaskCluster(false);
		this.setMaxError(0.05);
		this.setCodeEmbedData(true);

		this.determineClassification();
		this.generateFilenames(csvFile);
		this.generateSettings();
		this.generateSourceData(sourceData);
		...
		this.generateProcess(backwardWindow, forwardWindow, prediction, predictField);
		this.generateCode();

		// override raw_file to be the processed file
		this.script.getProperties().setProperty(ScriptProperties.HEADER_DATASOURCE_RAW_FILE, AnalystWizard.FILE_PRE);
	 ...
	 *            The file format.
	 */
	public void wizard(final File analyzeFile, final boolean b, final AnalystFileFormat format) {

		this.script.setBasePath(analyzeFile.getParent());
		this.script.getProperties().setProperty(ScriptProperties.HEADER_DATASOURCE_SOURCE_HEADERS, b);
		this.script.getProperties().setProperty(ScriptProperties.HEADER_DATASOURCE_RAW_FILE, analyzeFile);
	 ...
	 *            The file format.
	 */
	public void wizard(final URL url, final File saveFile, final File analyzeFile, final boolean b,
