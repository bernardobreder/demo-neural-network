...
public class EncogCodeGeneration {
	...
	public void generate(final EncogAnalyst analyst) {
		...
		if (this.generator instanceof ProgramGenerator) {
			...
			final String trainingID = analyst.getScript().getProperties()
			...
			this.generate(methodFile, trainingFile);
		} else {
	...
	public void generate(final File method, final File data) {
		...
		this.program
				.addComment("Code generated by Encog v" + Encog.getInstance().getProperties().get(Encog.ENCOG_VERSION));
		...
		final EncogProgramNode mainClass = this.program.createClass("EncogExample");
		...
		if (method != null) {
			createNetworkFunction = this.generateForMethod(mainClass, method);
		}
		...
		if (data != null) {
			if (!(this.generator instanceof GenerateEncogJavaScript)) {
				mainFunction.createFunctionCall("createTraining", "MLDataSet", "training");
			}
		}
		mainFunction.addComment("Network and/or data is now loaded, you can add code to train, evaluate, etc.");
	 ...
	 */
	private EncogProgramNode generateForMethod(final EncogProgramNode mainClass, final File method) {

		if (this.embedData) {
			final MLEncodable encodable = (MLEncodable) EncogDirectoryPersistence.loadObject(method);
