...
public class GenerateMQL4 extends AbstractTemplateGenerator {
	...
	private void processCalc() {
		...
		final int barsNeeded = Math.abs(this.getAnalyst().determineMinTimeSlice());

		final int inputCount = this.getAnalyst().determineInputCount();
		final int outputCount = this.getAnalyst().determineOutputCount();
		...
		int idx = 0;
		for (final AnalystField field : this.getAnalyst().getScript().getNormalize().getNormalizedFields()) {
			if (field.isInput()) {

				final DataField df = this.getAnalyst().getScript().findDataField(field.getName());
				String str;
		...
		this.addLine("ExtMapBuffer1[pos] = DeNorm(output[0]" + "," + firstOutputField.getNormalizedHigh() + ","
	...
	private void processHeaders() {
		final DataField[] fields = this.getAnalyst().getScript().getFields();
	...
	private void processMainBlock() {
		final EncogAnalyst analyst = this.getAnalyst();
	...
	private void processObtain() {
		...
		for (final DataField field : fields) {
			...
			if (!df.getName().equalsIgnoreCase("time") && !df.getName().equalsIgnoreCase("prediction")) {
				final String str = EngineArray.replace(df.getSource(), "##", "pos");
				if (lastLine != null) {
