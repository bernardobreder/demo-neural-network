...
public class ChainRuleWorker implements EngineTask {
	...
	public ChainRuleWorker(FlatNetwork theNetwork, MLDataSet theTraining, int theLow, int theHigh) {
		...
		this.totDeriv = new double[this.weightCount];
		this.gradients = new double[this.weightCount];
	...
	public void run() {
		...
		for (int i = this.low; i <= this.high; i++) {
			...
			this.process(this.outputNeuron, derivative, this.pair.getInputArray(), this.pair.getIdealArray());
	...
	private void process(int outputNeuron, double[] derivative, final double[] input, final double[] ideal) {
		...
		for (int i = 0; i < this.actual.length; i++) {
			...
			if (i == outputNeuron) {
				this.layerDelta[i] = this.flat.getActivationFunctions()[0].derivativeFunction(this.layerSums[i],
						this.layerOutput[i]);
			} else {
				this.layerDelta[i] = 0;
			}
	...
	private void processLevel(final int currentLevel, double[] derivative) {
		...
		final ActivationFunction activation = this.flat.getActivationFunctions()[currentLevel + 1];
