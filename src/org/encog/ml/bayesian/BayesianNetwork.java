...
public class BayesianNetwork extends BasicML implements MLClassification, MLResettable, Serializable, MLError {
	 ...
	 *
	 * @param label
	 *            The label to create this event as.
	 * @param options
	 *            The options, or states, that this event can have.
	 ...
	 *
	 * @param label
	 *            The label of the event to create.
	 * @param options
	 *            The states that the event can have.
	 * @return The newly created event.
	 */
	 ...
	 *
	 * @param parentEventLabel
	 *            The parent event.
	 * @param childEventLabel
	 *            The child event.
	 */
	 ...
	 *
	 * @param line
	 *            The string to define events and relations.
	 */
	public void setContents(String line) {
		...
		for (ParsedProbability prob : list) {
			...
			if (e == null) {
				List<BayesianChoice> cl = new ArrayList<>();
	 ...
	 *
	 * @param previousHead
	 *            The previous head, as we traverse the list.
	 * @param a
	 *            The event to check.
	 * @param goal
	 *            The goal.
	 * @param searched
	 *            List of events searched.
	 * @param given
	 *            Given events.
	 * @return True if conditionally independent.
	 */
	private boolean isCondIndependent(boolean previousHead, BayesianEvent a, BayesianEvent goal,
			Set<BayesianEvent> searched, BayesianEvent... given) {
		...
		for (BayesianEvent e : a.getChildren()) {
			if (!searched.contains(e) || !this.isGiven(given, a)) {
				searched.add(e);
				if (!this.isCondIndependent(true, e, goal, searched, given)) {
					return false;
				}
			}
		}
		...
		for (BayesianEvent e : a.getParents()) {
			if (!searched.contains(e)) {
				searched.add(e);
				if (!previousHead || this.isGivenOrDescendant(given, a)) {
					if (!this.isCondIndependent(false, e, goal, searched, given)) {
						return false;
					}
				}
			}
	...
	public boolean isCondIndependent(BayesianEvent a, BayesianEvent b, BayesianEvent... given) {
		Set<BayesianEvent> searched = new HashSet<>();
	 ...
	 *
	 * @param line
	 *            The line to define the probability.
	 */
	...
	public int classify(MLData input) {
		...
		for (int i = 0; i < outcomeEvent.getChoices().size(); i++) {
			...
			this.query.execute();
			this.classificationProbabilities[i] = this.query.getProbability();
		}
