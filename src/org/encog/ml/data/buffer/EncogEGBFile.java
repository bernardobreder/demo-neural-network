...
public class EncogEGBFile {
	...
	public void addColumn(final int col, final boolean isInput) {
		try {
			...
			// allocate buffers
			final ByteBuffer readBuffer = ByteBuffer.allocate(EncogEGBFile.DOUBLE_SIZE * 1024);
			final ByteBuffer writeBuffer = ByteBuffer.allocate(EncogEGBFile.DOUBLE_SIZE * 1024);
			...
			// adjust file size
			this.raf.setLength((this.numberOfRecords * this.recordSize) + EncogEGBFile.HEADER_SIZE);
	...
	public void addRow(final int row) {
		try {
			...
			for (int i = this.numberOfRecords - 1; i >= row; i--) {
				...
				final int t = EncogEGBFile.HEADER_SIZE + (this.recordSize * i) + this.recordSize;
			...
			this.recordBuffer.flip();
			this.fc.write(this.recordBuffer, EncogEGBFile.HEADER_SIZE + (this.recordSize * row));
	...
	private int calculateIndex(final int row, final int col) {
		return EncogEGBFile.HEADER_SIZE + (row * this.recordSize) + (col * EncogEGBFile.DOUBLE_SIZE);
	}
	...
	public void create(final int theInputCount, final int theIdealCount) {
		try {
			...
			final double[] input = new double[this.inputCount];
			final double[] ideal = new double[this.idealCount];
	...
	public void deleteRow(final int row) {
		try {
			for (int i = row; i < this.numberOfRecords - 1; i++) {
				final int s = EncogEGBFile.HEADER_SIZE + (this.recordSize * i) + this.recordSize;
