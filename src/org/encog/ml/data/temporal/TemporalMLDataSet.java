...
public class TemporalMLDataSet extends BasicNeuralDataSet implements Serializable {
	 ...
	 */
	private final List<TemporalDataDescription> descriptions = new ArrayList<>();

	/**
	 * The temporal points at which we have data.
	 */
	private final List<TemporalPoint> points = new ArrayList<>();
	 ...
	 */
	public TemporalMLDataSet(final int inputWindowSize, final int predictWindowSize) {
	...
	public void addDescription(final TemporalDataDescription desc) {
		if (this.points.size() > 0) {
			final String str = "Can't add anymore descriptions, there are " + "already temporal points defined.";
	...
	public TemporalPoint createPoint(final Date when) {
		final int sequence = this.getSequenceFromDate(when);
	...
	public void generate() {
		this.sortPoints();
		// add one to the start index so we are "one ahead", needed to calculate
		// DELTA, if that encoding is chosen.
		final int start = this.calculateStartIndex() + 1;
		final int setSize = this.calculateActualSetSize();
		final int range = start + setSize - this.predictWindowSize - this.inputWindowSize;

		for (int i = start; i < range; i++) {
			final BasicMLData input = this.generateInputNeuralData(i);
			final BasicMLData ideal = this.generateOutputNeuralData(i + this.inputWindowSize);
	...
	public BasicNeuralData generateOutputNeuralData(final int index) {
		if (index + this.predictWindowSize > this.points.size()) {
			...
			final String str = "Can't generate prediction temporal data " + "beyond the end of provided data.";
	...
	private double getDataRAW(final TemporalDataDescription desc, final int index) {
		// Note: The reason that we subtract 1 from the index is because we are
		// always one ahead.
		// This allows the DELTA change formatter to work. DELTA change requires
		// two timeslices,
		// so we have to be one ahead. RAW only requires one, so we shift
		// backwards.
