...
public class ColumnDefinition implements Serializable {
	...
	private void analyzeOrdinal(String value) {
		if (!this.classes.contains(value)) {
			throw (new EncogError(
					"You must predefine any ordinal values (in order). Undefined ordinal value: " + value));
		}
	}
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof ColumnDefinition) {
			...
			// Compares floating point values with exact equality, with no
			// delta.
			result = ((this.name == that.name) || ((null != this.name) && this.name.equals(that.name)))
					&& (this.dataType == that.dataType) && Double.valueOf(this.low).equals(that.low)
					&& Double.valueOf(this.high).equals(that.high) && Double.valueOf(this.mean).equals(that.mean)
					&& Double.valueOf(this.sd).equals(that.sd) && (this.count == that.count)
					&& (this.index == that.index) && this.classes.equals(that.classes);
