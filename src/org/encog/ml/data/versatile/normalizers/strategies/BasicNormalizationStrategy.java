...
/**
 * Provides a basic normalization strategy that will work with most models built
 * into Encog. This is often used as a starting point for building more
 * customized models, as this normalizer works mainly by using maps to define
 * which normalizer to use for what data type.
 */
public class BasicNormalizationStrategy implements NormalizationStrategy {
	...
	public BasicNormalizationStrategy(double inputLow, double inputHigh, double outputLow, double outputHigh) {
		this.assignInputNormalizer(ColumnType.continuous, new RangeNormalizer(inputLow, inputHigh));
		this.assignInputNormalizer(ColumnType.nominal, new OneOfNNormalizer(inputLow, inputHigh));
		this.assignInputNormalizer(ColumnType.ordinal, new RangeOrdinal(inputLow, inputHigh));

		this.assignOutputNormalizer(ColumnType.continuous, new RangeNormalizer(outputLow, outputHigh));
		this.assignOutputNormalizer(ColumnType.nominal, new OneOfNNormalizer(outputLow, outputHigh));
		this.assignOutputNormalizer(ColumnType.ordinal, new RangeOrdinal(outputLow, outputHigh));
	}
	...
	/**
	 * Find a normalizer for the specified column definition, and if it is input
	 * or output.
	 *
	 * @param colDef
	 ...
	 */
	@Override
	public int normalizeColumn(ColumnDefinition colDef, boolean isInput, String value, double[] outputData,
			int outputColumn) {
		Normalizer norm = this.findNormalizer(colDef, isInput);
	 ...
	 */
	@Override
	public int normalizeColumn(ColumnDefinition colDef, boolean isInput, double value, double[] outputData,
			int outputColumn) {
		Normalizer norm = this.findNormalizer(colDef, isInput);
		return norm.normalizeColumn(colDef, value, outputData, outputColumn);
	}
