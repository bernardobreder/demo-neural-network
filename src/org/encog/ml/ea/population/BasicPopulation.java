...
public class BasicPopulation extends BasicML implements Population, Serializable {
	 ...
	 *            The population size.
	 * @param theGenomeFactory
	 *            factory that can be used to store create genomes.
	 */
	public BasicPopulation(final int thePopulationSize, final GenomeFactory theGenomeFactory) {
	...
	public void purgeInvalidGenomes() {
		...
		while (speciesNum < this.getSpecies().size()) {
			...
			while (genomeNum < species.getMembers().size()) {
				...
				if (Double.isInfinite(genome.getScore()) || Double.isInfinite(genome.getAdjustedScore())
						|| Double.isNaN(genome.getScore()) || Double.isNaN(genome.getAdjustedScore())) {
