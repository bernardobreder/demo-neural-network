...
public abstract class ThresholdSpeciation implements Speciation, Serializable {
	 ...
	 */
	private void divideByFittestSpecies(final List<Species> speciesCollection, final double totalSpeciesScore) {
		...
		for (final Object element : speciesArray) {
			...
			int share = (int) Math.round(
					(species.getOffspringShare() / totalSpeciesScore) * this.owner.getPopulation().getPopulationSize());
			...
			else if ((species.getGensNoImprovement() > this.numGensAllowedNoImprovement) && (species != bestSpecies)) {
				this.removeSpecies(species);
			} else {
	...
	public void removeSpecies(Species species) {
		if (species != this.findBestSpecies()) {
			if (this.population.getSpecies().size() > 1) {
	...
	private void divideEven(final List<Species> speciesCollection) {
		...
		for (final Species species : speciesCollection) {
			final int share = (int) Math.round(ratio * this.owner.getPopulation().getPopulationSize());
			species.setOffspringCount(share);
		}
	}
	...
	private void levelOff() {
		...
		if (diff < 0) {
			...
			while ((diff != 0) && (index > 0)) {
				...
				final int t = Math.min(species.getOffspringCount(), Math.abs(diff));
	...
	public void performSpeciation(List<Genome> genomeList) {
		final List<Genome> newGenomeList = this.resetSpecies(genomeList);
	...
	private List<Genome> resetSpecies(List<Genome> inputGenomes) {
		final List<Genome> result = new ArrayList<>();
	 ...
	 */
	public void setNumGensAllowedNoImprovement(final int numGensAllowedNoImprovement) {
	...
	private void speciateAndCalculateSpawnLevels(final List<Genome> genomes) {
		...
		for (final Genome g : genomes) {
			...
			for (final Species s : speciesCollection) {
				final double compatibility = this.getCompatibilityScore(genome, s.getLeader());
