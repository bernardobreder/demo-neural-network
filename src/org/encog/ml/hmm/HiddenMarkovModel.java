 ...
 * such as interest rate and the level of the S&amp;P500. The HMM learns what
 * state we are in by seeing how the observations change over time.
 *
...
public class HiddenMarkovModel extends BasicML implements MLStateSequence, Serializable, Cloneable {
	...
	public HiddenMarkovModel clone() throws CloneNotSupportedException {
		final HiddenMarkovModel hmm = this.cloneStructure();
		...
		for (int i = 0; i < this.transitionProbability.length; i++) {
			hmm.transitionProbability[i] = this.transitionProbability[i].clone();
		}
	...
	public HiddenMarkovModel cloneStructure() {
		...
		if (this.isDiscrete()) {
			hmm = new HiddenMarkovModel(this.getStateCount(), this.items);
		} else {
			hmm = new HiddenMarkovModel(this.getStateCount());
		}
	...
	public double probability(final MLDataSet seq, final int[] states) {
		...
		for (int i = 0; i < (states.length - 1); i++) {
			probability *= this.getStateDistribution(states[i]).probability(oseqIterator.next())
					* this.getTransitionProbability(states[i], states[i + 1]);
		}

		return probability
				* this.getStateDistribution(states[states.length - 1]).probability(seq.get(states.length - 1));
	}
	...
	public void setTransitionProbability(final int i, final int j, final double value) {
