...
public class PersistHMM implements EncogPersistor {
	...
	public final Object read(final InputStream is) {
		...
		List<StateDistribution> distributions = new ArrayList<>();
		...
		while ((section = in.readNextSection()) != null) {
			...
			if (section.getSectionName().equals("HMM") && section.getSubSectionName().equals("CONFIG")) {
				...
				if (params.containsKey(HiddenMarkovModel.TAG_ITEMS)) {
					EncogFileSection.parseIntArray(params, HiddenMarkovModel.TAG_ITEMS);
				}
				pi = section.parseDoubleArray(params, HiddenMarkovModel.TAG_PI);
				transitionProbability = EncogFileSection.parseMatrix(params, HiddenMarkovModel.TAG_TRANSITION);
				...
				if ("ContinousDistribution".equals(t)) {
					...
					Matrix cova = EncogFileSection.parseMatrix(params, HiddenMarkovModel.TAG_COVARIANCE);
					ContinousDistribution dist = new ContinousDistribution(mean, cova.getData());
					distributions.add(dist);
				} else if ("DiscreteDistribution".equals(t)) {
					Matrix prob = EncogFileSection.parseMatrix(params, HiddenMarkovModel.TAG_PROBABILITIES);
