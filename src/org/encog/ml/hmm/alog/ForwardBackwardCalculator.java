...
public class ForwardBackwardCalculator {
	 ...
	 *            The sequence to use.
	 * @param hmm
	 *            THe hidden markov model to use.
	 */
	public ForwardBackwardCalculator(final MLDataSet oseq, final HiddenMarkovModel hmm) {
	 ...
	 *            The state.
	 */
	protected void computeAlphaInit(final HiddenMarkovModel hmm, final MLDataPair o, final int i) {
	 ...
	 *            Thr column.
	 */
	protected void computeAlphaStep(final HiddenMarkovModel hmm, final MLDataPair o, final int t, final int j) {
		double sum = 0.;
	 ...
	 *            The hidden markov model.
	 * @param o
	 *            THe data par to compute.
	 * @param t
	 *            THe matrix row.
	 * @param i
	 *            THe matrix column.
	 */
	protected void computeBetaStep(final HiddenMarkovModel hmm, final MLDataPair o, final int t, final int i) {
		double sum = 0.;
	 ...
	 *            The flags.
	 */
	private void computeProbability(final MLDataSet oseq, final HiddenMarkovModel hmm,
			final EnumSet<Computation> flags) {
		this.probability = 0.;
		...
		if (flags.contains(Computation.ALPHA)) {
			...
			for (int i = 0; i < hmm.getStateCount(); i++) {
				this.probability += hmm.getPi(i) * hmm.getStateDistribution(i).probability(oseq.get(0))
						* this.beta[0][i];
			}
		}
	}
