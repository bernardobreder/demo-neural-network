...
public class KullbackLeiblerDistanceCalculator {
	...
	public double distance(final HiddenMarkovModel hmm1, final HiddenMarkovModel hmm2) {
		double distance = 0.;

		for (int i = 0; i < this.sequenceCount; i++) {

			final MLDataSet oseq = new MarkovGenerator(hmm1).observationSequence(this.len);

			distance += (new ForwardBackwardScaledCalculator(oseq, hmm1).lnProbability()
					- new ForwardBackwardScaledCalculator(oseq, hmm2).lnProbability()) / this.len;
		}
