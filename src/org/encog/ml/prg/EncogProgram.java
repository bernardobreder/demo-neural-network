...
public class EncogProgram extends BasicGenome implements MLRegression, MLError {
	 ...
	 *            The variable holder.
	 */
	public EncogProgram(final EncogProgramContext theContext, final EncogProgramVariables theVariables) {
	...
	public MLData compute(final MLData input) {
		...
		final VariableMapping resultMapping = this.getResultType();
		...
		if (!success) {
			throw new EARuntimeError("EncogProgram produced " + v.getExpressionType().toString() + " but "
					+ resultMapping.getVariableType().toString() + " was expected.");
		}
	 ...
	 */
	public void replaceNode(final ProgramNode replaceThisNode, final ProgramNode replaceWith) {
	 ...
	 */
	public int selectRandomVariable(final Random rnd, final List<ValueType> desiredTypes) {
		...
		if (selectionSet.size() == 0 && desiredTypes.contains(ValueType.intType)) {
			final List<ValueType> floatList = new ArrayList<>();
		...
		final VariableMapping selected = selectionSet.get(rnd.nextInt(selectionSet.size()));
