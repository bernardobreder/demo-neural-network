...
public class EncogProgramContext implements Serializable {
	 ...
	 * defines the set of functions and operators that a program might use. For
	 * an Encog Program all operators are treated as functions internally. A
	 ...
	 * variables that make up an algebraic expression.
	 */
	private final List<VariableMapping> definedVariables = new ArrayList<>();

	/**
	 * Lookup map for the defined variables.
	 */
	private final Map<String, VariableMapping> map = new HashMap<>();
	 ...
	 */
	public EncogProgramContext(final CSVFormat theFormat, final FunctionFactory theFunctions) {
	 ...
	 */
	public ProgramNode cloneBranch(final EncogProgram targetProgram, final ProgramNode sourceBranch) {
		...
		// create any subnodes
		final ProgramNode[] args = new ProgramNode[sourceBranch.getChildNodes().size()];
		for (int i = 0; i < args.length; i++) {
			args[i] = this.cloneBranch(targetProgram, (ProgramNode) sourceBranch.getChildNodes().get(i));
		}

		final ProgramNode result = targetProgram.getContext().getFunctions().factorProgramNode(name, targetProgram,
				args);
	 ...
	 *            The variable type.
	 */
	public void defineVariable(final String theName, final ValueType theVariableType) {
	 ...
	 *            type.
	 */
	public void defineVariable(final String theName, final ValueType theVariableType, final int theEnumType,
			final int theEnumValueCount) {
		final VariableMapping mapping = new VariableMapping(theName, theVariableType, theEnumType, theEnumValueCount);
	 ...
	 */
	public List<VariableMapping> findVariablesByTypes(final List<ValueType> desiredTypes) {
		final List<VariableMapping> result = new ArrayList<>();
