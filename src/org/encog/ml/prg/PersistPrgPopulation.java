...
public class PersistPrgPopulation implements EncogPersistor {
	...
	public Object read(final InputStream is) {
		...
		while ((section = in.readNextSection()) != null) {
			...
			} else if (section.getSectionName().equals("BASIC") && section.getSubSectionName().equals("EPL-OPCODES")) {
				for (final String line : section.getLines()) {
					final List<String> cols = EncogFileSection.splitColumns(line);
			...
			} else if (section.getSectionName().equals("BASIC") && section.getSubSectionName().equals("EPL-SYMBOLIC")) {
				boolean first = true;
				for (final String line : section.getLines()) {
					if (!first) {
						final List<String> cols = EncogFileSection.splitColumns(line);
						...
						final VariableMapping mapping = new VariableMapping(name, vt, enumType, enumCount);
	...
	public void save(final OutputStream os, final Object obj) {
		...
		for (final ProgramExtensionTemplate temp : pop.getContext().getFunctions().getOpCodes()) {
