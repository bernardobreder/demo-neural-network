...
public abstract class BasicTemplate implements ProgramExtensionTemplate {
	 ...
	 * The parameters.
	 */
	private final List<ParamTemplate> params = new ArrayList<>();
	 ...
	 */
	public BasicTemplate(final int thePrecedence, final String theSignature, final NodeType theType,
			final boolean isVariable, final int theDataSize) {
		...
		if (theSignature.trim().equals("(")) {
			...
			while (!done) {
				if (parser.peek() == ')') {
					...
					final ParamTemplate temp = this.readParam(parser);
