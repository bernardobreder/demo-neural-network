...
public class SubtreeCrossover implements EvolutionaryOperator {
	 ...
	 *            The level holder.
	 */
	private void findNode(final Random rnd, final ProgramNode parentNode, final List<ValueType> types,
			final LevelHolder holder) {
		if (holder.getCurrentLevel() == 0) {
			...
			for (int i = 0; i < parentNode.getTemplate().getChildNodeCount(); i++) {
				...
				final List<ValueType> childTypes = parentNode.getTemplate().getParams().get(i)
						.determineArgumentTypes(types);
				this.findNode(rnd, childNode, childTypes, holder);
			}
		}
	}
	...
	public void performOperation(final Random rnd, final Genome[] parents, final int parentIndex,
		...
		while (!done) {
			...
			this.findNode(rnd, parent1.getRootNode(), types, holder1);
			this.findNode(rnd, parent2.getRootNode(), types, holder2);
			...
			if (LevelHolder.compatibleTypes(holder1.getTypes(), holder2.getTypes())) {
				...
				final ProgramNode newInsert = context.cloneBranch(result, p2Node);
