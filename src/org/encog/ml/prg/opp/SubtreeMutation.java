...
public class SubtreeMutation implements EvolutionaryOperator {
	 ...
	 *            The maximum depth.
	 */
	public SubtreeMutation(final EncogProgramContext theContext, final int theMaxDepth) {
	 ...
	 *            The level holder.
	 */
	private void findNode(final Random rnd, final EncogProgram result, final ProgramNode parentNode,
			final List<ValueType> types, final int[] globalIndex) {
		if (globalIndex[0] == 0) {
			globalIndex[0]--;

			final ProgramNode newInsert = this.generator.createNode(rnd, result, this.maxDepth, types);
