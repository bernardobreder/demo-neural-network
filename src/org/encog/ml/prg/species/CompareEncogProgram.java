...
public class CompareEncogProgram {
	...
	/**
	 * Compare program 1 and 2 node for node. Lower values mean more similar
	 * genomes.
	 *
	 * @param prg1
	 *            The first program.
	...
	public double compare(final EncogProgram prg1, final EncogProgram prg2) {
		return this.compareNode(0, prg1.getRootNode(), prg2.getRootNode());
	}
	 ...
	 * @return The result.
	 */
	private double compareNode(final double result, final ProgramNode node1, final ProgramNode node2) {
		...
		for (int i = 0; i < childNodeCount; i++) {
			if (i < node1Size && i < node2Size) {
				...
				newResult = this.compareNode(newResult, childNode1, childNode2);
			} else {
				newResult++;
			}
		}
