...
public class RewriteAlgebraic implements RewriteRule {
	...
	private ProgramNode createNumericConst(final EncogProgram prg, final int v) {
		final ProgramNode result = prg.getFunctions().factorProgramNode("#const", prg, new ProgramNode[] {});
	...
	private ProgramNode internalRewrite(final ProgramNode parent) {
		...
		for (int i = 0; i < rewrittenParent.getChildNodes().size(); i++) {
			final ProgramNode childNode = (ProgramNode) rewrittenParent.getChildNodes().get(i);
			final ProgramNode rewriteChild = this.internalRewrite(childNode);
	...
	public boolean rewrite(final Genome g) {
		...
		final ProgramNode rewrittenRoot = this.internalRewrite(node);
			...
			if (child2.getName().equals("#const")) {
				...
				if (v.isFloat()) {
					...
					if (v2 < 0) {
						...
						parent = parent.getOwner().getContext().getFunctions().factorProgramNode("+", parent.getOwner(),
	...
	private ProgramNode tryPlusNeg(ProgramNode parent) {
		if (parent.getName().equals("+") && parent.getChildNodes().size() == 2) {
			...
			if (child2.getName().equals("-") && child2.getChildNodes().size() == 1) {
				parent = parent.getOwner().getContext().getFunctions().factorProgramNode("-", parent.getOwner(),
						new ProgramNode[] { child1, child2.getChildNode(0) });
	...
	private ProgramNode tryPowerZero(final ProgramNode parent) {
		if (parent.getTemplate() == StandardExtensions.EXTENSION_POWER
			...
			if (this.isConstValue(child1, 0)) {
				return this.createNumericConst(parent.getOwner(), 1);
			}
			if (this.isConstValue(child0, 0)) {
	...
	private ProgramNode tryVarOpVar(ProgramNode parent) {
		if (parent.getChildNodes().size() == 2 && parent.getName().length() == 1
			...
			if (child1.getName().equals("#var") && child2.getName().equals("#var")) {
				if (child1.getData()[0].toIntValue() == child2.getData()[0].toIntValue()) {
					...
					case '-':
						...
						break;
					case '+':
						parent = parent.getOwner().getFunctions().factorProgramNode("*", parent.getOwner(),
								new ProgramNode[] { this.createNumericConst(parent.getOwner(), 2), child1 });
						break;
					case '*':
						parent = parent.getOwner().getFunctions().factorProgramNode("^", parent.getOwner(),
								new ProgramNode[] { child1, this.createNumericConst(parent.getOwner(), 2) });
						break;
					case '/':
	...
	private ProgramNode tryZeroMul(final ProgramNode parent) {
		if (parent.getTemplate() == StandardExtensions.EXTENSION_MUL) {
			...
			if (this.isConstValue(child1, 0) || this.isConstValue(child2, 0)) {
				this.rewritten = true;
				return this.createNumericConst(parent.getOwner(), 0);
			}
		}
