...
public class PersistSVM implements EncogPersistor {
	...
	public Object read(final InputStream is) {
		...
		while ((section = in.readNextSection()) != null) {
			...
			if (section.getSectionName().equals("SVM") && section.getSubSectionName().equals("SVM-PARAM")) {
				...
				result.getParams().cache_size = EncogFileSection.parseDouble(params, PersistSVM.PARAM_CACHE_SIZE);
				...
				result.getParams().kernel_type = EncogFileSection.parseInt(params, PersistSVM.PARAM_KERNEL_TYPE);
				result.getParams().nr_weight = EncogFileSection.parseInt(params, PersistSVM.PARAM_NUM_WEIGHT);
				...
				result.getParams().svm_type = EncogFileSection.parseInt(params, PersistSVM.PARAM_SVM_TYPE);
				result.getParams().weight = section.parseDoubleArray(params, PersistSVM.PARAM_WEIGHT);
				result.getParams().weight_label = EncogFileSection.parseIntArray(params, PersistSVM.PARAM_WEIGHT_LABEL);
			} else if (section.getSectionName().equals("SVM") && section.getSubSectionName().equals("SVM-MODEL")) {
				try {
					final StringReader rdr = new StringReader(section.getLinesAsString());
	...
	public void save(final OutputStream os, final Object obj) {
		...
		out.writeProperty(PersistSVM.PARAM_CACHE_SIZE, svm2.getParams().cache_size);
		...
		out.writeProperty(PersistSVM.PARAM_KERNEL_TYPE, svm2.getParams().kernel_type);
		out.writeProperty(PersistSVM.PARAM_NUM_WEIGHT, svm2.getParams().nr_weight);
		out.writeProperty(PersistSVM.PARAM_NU, svm2.getParams().nu);
		out.writeProperty(PersistSVM.PARAM_P, svm2.getParams().p);
		out.writeProperty(PersistSVM.PARAM_PROBABILITY, svm2.getParams().probability);
		...
		out.writeProperty(PersistSVM.PARAM_WEIGHT_LABEL, svm2.getParams().weight_label);
