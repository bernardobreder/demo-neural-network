...
public class SVM extends BasicML implements MLRegression, MLClassification, MLError {
	...
	public SVM(final int theInputCount, final boolean regression) {
		this(theInputCount, regression ? SVMType.EpsilonSupportVectorRegression : SVMType.SupportVectorClassification,
	 ...
	 *            The SVM kernal type.
	 */
	public SVM(final int theInputCount, final SVMType svmType, final KernelType kernelType) {
		...
		this.params.gamma = 1.0 / this.inputCount;
	...
	public int classify(final MLData input) {
		...
		final svm_node[] formattedInput = this.makeSparse(input);
