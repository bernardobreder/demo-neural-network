...
public class HybridStrategy implements Strategy {
	...
	/**
	 * The minimum improvement before the alternate training algorithm is
	 * considered.
	 */
	private final double minImprovement;

	/**
	 * The number of minimal improvement to tolerate before the alternate
	...
	/**
	 * Construct a hybrid strategy with the default minimum improvement and
	 * toleration cycles.
	 *
	 * @param altTrain
	 *            The alternative training strategy.
	 */
	public HybridStrategy(final MLTrain altTrain) {
		this(altTrain, HybridStrategy.DEFAULT_MIN_IMPROVEMENT, HybridStrategy.DEFAULT_TOLERATE_CYCLES,
	 ...
	 * @param tolerateMinImprovement
	 *            The number of cycles to tolerate the minimum improvement for.
	 * @param alternateCycles
	 *            How many cycles should the alternate training algorithm be
	 *            used for.
	 */
	public HybridStrategy(final MLTrain altTrain, final double minImprovement, final int tolerateMinImprovement,
			final int alternateCycles) {
	...
	public void postIteration() {
		if (this.ready) {
			...
			if ((this.lastImprovement > 0) || (Math.abs(this.lastImprovement) < this.minImprovement)) {
				this.lastHybrid++;
