...
public class GridStochasticProbability extends GridAbstractProbability {
	...
	public GridStochasticProbability(GridWorld theWorld, double theProbabilitySuccess, double theProbabilitySame,
			double theProbabilityLeft, double theProbabilityRight, double theProbabilityReverse) {
		super(theWorld);
	...
	/**
	 * @param probabilitySuccess
	 *            the probabilitySuccess to set
	 */
	...
	/**
	 * @param probabilitySame
	 *            the probabilitySame to set
	 */
	...
	/**
	 * @param probabilityLeft
	 *            the probabilityLeft to set
	 */
	...
	/**
	 * @param probabilityRight
	 *            the probabilityRight to set
	 */
	...
	/**
	 * @param probabilityReverse
	 *            the probabilityReverse to set
	 */
	...
	public Set<SuccessorState> determineSuccessorStates(State state, Action action) {
		...
		Set<SuccessorState> result = new TreeSet<>();

		if (action != null) {
			// probability of successful action
			if (this.probabilitySuccess > Encog.DEFAULT_DOUBLE_EQUAL) {
				State newState = this.determineActionState((GridState) state, action);
				if (newState != null) {
			...
			if (this.probabilityLeft > Encog.DEFAULT_DOUBLE_EQUAL) {
				State newState = this.determineActionState((GridState) state, GridWorld.leftOfAction(action));
				if (newState != null) {
			...
			if (this.probabilityRight > Encog.DEFAULT_DOUBLE_EQUAL) {
				State newState = this.determineActionState((GridState) state, GridWorld.rightOfAction(action));
				if (newState != null) {
			...
			if (this.probabilityReverse > Encog.DEFAULT_DOUBLE_EQUAL) {
				State newState = this.determineActionState((GridState) state, GridWorld.reverseOfAction(action));
				if (newState != null) {
