 ...
 * network. ART1 was developed by Stephen Grossberg and Gail Carpenter. This
 * neural network type supports only bipolar input. The ART1 neural network is
 * trained as it is used. New patterns are presented to the ART1 network, and
 * they are classified into either new, or existing, classes. Once the maximum
 * number of classes have been used the network will report that it is out of
 * classes. ART1 neural networks are used for classification.
 *
 * There are essentially 2 layers in an ART1 network. The first, named the F1
 * layer, acts as the input. The F1 layer receives bipolar patterns that the
 * network is to classify. The F2 layer specifies the maximum number classes
 * that the ART1 network can recognize.
 *
...
public class ART1 extends ART implements MLResettable, MLClassification {
	...
	/**
	 * This is the value that is returned if there is no winner. This value is
	 * generally set to the number of classes, plus 1.
	 */
	...
	public void adjustWeights() {
		...
		for (int i = 0; i < this.f1Count; i++) {
			if (this.outputF1.getBoolean(i)) {
				magnitudeInput = this.magnitude(this.outputF1);
				this.weightsF1toF2.set(i, this.winner, 1);
				this.weightsF2toF1.set(this.winner, i, this.l / (this.l - 1 + magnitudeInput));
			} else {
	 ...
	 */
	public void compute(final BiPolarNeuralData input, final BiPolarNeuralData output) {
		int i;
		...
		do {
			...
			if (this.winner != this.noWinner) {
				...
				magnitudeInput1 = this.magnitude(input);
				magnitudeInput2 = this.magnitude(this.outputF1);
	...
	private void computeF1(final BiPolarNeuralData input) {
		...
		for (int i = 0; i < this.f1Count; i++) {
			sum = this.weightsF1toF2.get(i, this.winner) * (this.outputF2.getBoolean(this.winner) ? 1 : 0);
		...
		for (i = 0; i < this.f2Count; i++) {
			if (!this.inhibitF2[i]) {
				...
				for (j = 0; j < this.f1Count; j++) {
					sum += this.weightsF2toF1.get(i, j) * (this.outputF1.getBoolean(j) ? 1 : 0);
				}
				if (sum > maxOut) {
					maxOut = sum;
					this.winner = i;
	...
	/**
	 * @return This is the value that is returned if there is no winner. This
	 *         value is generally set to the index of the last classes, plus 1.
	 *         For example, if there were 3 classes, the network would return
	 *         0-2 to represent what class was found, in this case the no winner
	 *         property would be set to 3.
	 */
	...
	public void reset(final int seed) {
		for (int i = 0; i < this.f1Count; i++) {
			for (int j = 0; j < this.f2Count; j++) {
				...
				this.weightsF2toF1.set(j, i, this.l / (this.l - 1 + this.f1Count) - 0.1);
			}
		}
	}
