 ...
 * by Bart Kosko in 1988. The BAM is a recurrent neural network that works
 * similarly that allows patterns of different lengths to be mapped
 * bidirectionally to other patterns. This allows it to act as almost a two-way
 * hash map. During training the BAM is fed pattern pairs. The two halves of
 * each pattern do not have to be the to be of the same length. However all
 * patterns must be of the same overall structure. The BAM can be fed a
 * distorted pattern on either side and will attempt to map to the correct
 * value.
 *
 * @author jheaton
 *
...
public class BAM extends BasicML {
	...
	public void addPattern(final MLData inputPattern, final MLData outputPattern) {
		...
		for (int i = 0; i < this.f1Count; i++) {
			for (int j = 0; j < this.f2Count; j++) {
				weight = (int) (inputPattern.getData(i) * outputPattern.getData(j));
	 ...
	 */
	private double getWeight(final Matrix matrix, final MLData input, final int x, final int y) {
	 ...
	 */
	private boolean propagateLayer(final Matrix matrix, final MLData input, final MLData output) {
		int i, j;
		int sum, out = 0;
		boolean stable;
		...
		for (i = 0; i < output.size(); i++) {
			...
			for (j = 0; j < input.size(); j++) {
				sum += this.getWeight(matrix, input, i, j) * input.getData(j);
			}
			if (sum != 0) {
				if (sum < 0) {
					out = -1;
