...
public class TrainInstar extends BasicTraining implements LearningRate {
	 ...
	 */
	public TrainInstar(final CPN theNetwork, final MLDataSet theTraining, final double theLearningRate,
	...
	private void initWeights() {
		...
		if (this.training.getRecordCount() != this.network.getInstarCount()) {
			throw new NeuralNetworkError("If the weights are to be set from the "
					+ "training data, then there must be one instar " + "neuron for each training element.");
		}
	...
	public void iteration() {
		...
		for (final MLDataPair pair : this.training) {
			...
			for (int i = 0; i < pair.getInput().size(); i++) {
				final double diff = pair.getInput().getData(i) - this.network.getWeightsInputToInstar().get(i, winner);
				distance += diff * diff;
			}
			...
			for (int j = 0; j < this.network.getInputCount(); j++) {
				final double delta = this.learningRate
						* (pair.getInput().getData(j) - this.network.getWeightsInputToInstar().get(j, winner));
