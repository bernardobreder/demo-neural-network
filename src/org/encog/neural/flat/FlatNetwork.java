...
public class FlatNetwork implements Serializable, Cloneable {
	 ...
	 */
	public FlatNetwork(final int input, final int hidden1, final int hidden2, final int output, final boolean tanh) {
	...
	protected void computeLayer(final int currentLayer) {
		...
		EngineArray.arrayCopy(this.layerOutput, outputIndex, this.layerOutput, offset,
	...
	public void decodeNetwork(final double[] data) {
		if (data.length != this.weights.length) {
			throw new EncogError("Incompatible weight sizes, can't assign length=" + data.length + " to length="
					+ this.weights.length);
		}
			...
			for (int j = layers.length - 1; j >= 0; j--) {
				if (layers[j].getContextFedBy() == layer) {
					...
					this.contextTargetOffset[index] = neuronIndex
							+ (layers[j].getTotalCount() - layers[j].getContextCount());
				}
