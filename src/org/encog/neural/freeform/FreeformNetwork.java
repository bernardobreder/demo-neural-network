 ...
 *
 */
public class FreeformNetwork extends BasicML
	 ...
	 */
	public static FreeformNetwork createElman(final int input, final int hidden1, final int output,
	...
	public static FreeformNetwork createFeedforward(final int input, final int hidden1, final int hidden2,
			final int output, final ActivationFunction af) {
		final FreeformNetwork network = new FreeformNetwork();
	...
	public FreeformNetwork(final BasicNetwork network) {
		...
		for (int currentLayerIndex = 0; currentLayerIndex < network.getLayerCount(); currentLayerIndex++) {
			...
			if (previousLayer != null) {
				this.connectLayersFromBasic(network, currentLayerIndex - 1, previousLayer, currentLayerIndex,
						currentLayer, currentLayerIndex, false);
			}

			// Add the bias neuron
			// The bias is added after connections so it has no inputs
			if (network.isLayerBiased(currentLayerIndex)) {
				final FreeformNeuron biasNeuron = this.neuronFactory.factorRegular(null);
	...
	public MLData compute(final MLData input) {
		...
		for (int i = 0; i < this.outputLayer.size(); i++) {
			final FreeformNeuron outputNeuron = this.outputLayer.getNeurons().get(i);
	 ...
	 *            The target layer.
	 */
	public void connectLayers(final FreeformLayer source, final FreeformLayer target) {
	 ...
	 */
	public void connectLayers(final FreeformLayer source, final FreeformLayer target,
			final ActivationFunction theActivationFunction, final double biasActivation, final boolean isRecurrent) {
		...
		for (final FreeformNeuron targetNeuron : target.getNeurons()) {
			...
			for (final FreeformNeuron sourceNeuron : source.getNeurons()) {
				final FreeformConnection connection = this.connectionFactory.factor(sourceNeuron, targetNeuron);
	 ...
	 */
	private void connectLayersFromBasic(final BasicNetwork network, final int fromLayerIdx, final FreeformLayer source,
			final int sourceIdx, final FreeformLayer target, final int targetIdx, final boolean isRecurrent) {
		...
		for (int targetNeuronIdx = 0; targetNeuronIdx < target.size(); targetNeuronIdx++) {
			for (int sourceNeuronIdx = 0; sourceNeuronIdx < source.size(); sourceNeuronIdx++) {
				final FreeformNeuron sourceNeuron = source.getNeurons().get(sourceNeuronIdx);
				final FreeformNeuron targetNeuron = target.getNeurons().get(targetNeuronIdx);
				...
				sourceNeuron.addOutput(connection);
				targetNeuron.addInput(connection);
				final double weight = network.getWeight(fromLayerIdx, sourceNeuronIdx, targetNeuronIdx);
	 ...
	 */
	public FreeformLayer createContext(final FreeformLayer source, final FreeformLayer target) {
		...
		for (int i = 0; i < source.size(); i++) {
			...
			if (neuron.isBias()) {
				...
				final FreeformNeuron contextNeuron = this.neuronFactory.factorContext(neuron);
	...
	public void decodeFromArray(final double[] encoded) {
		...
		while (queue.size() > 0) {
			...
			if (neuron.getInputSummation() != null) {
				for (final FreeformConnection connection : neuron.getInputSummation().list()) {
	 ...
	 *            The task.
	 */
	private void performConnectionTask(final Set<FreeformNeuron> visited, final FreeformNeuron parentNeuron,
			final ConnectionTask task) {
		visited.add(parentNeuron);

		// does this neuron have any inputs?
		if (parentNeuron.getInputSummation() != null) {
			// visit the inputs
			for (final FreeformConnection connection : parentNeuron.getInputSummation().list()) {
	 ...
	 *            The visited list.
	 * @param parentNeuron
	 *            The neuron to start with.
	 * @param task
	 *            The task to perform.
	 */
	private void performNeuronTask(final Set<FreeformNeuron> visited, final FreeformNeuron parentNeuron,
			final NeuronTask task) {
		visited.add(parentNeuron);
	 ...
	 *
	 * @param neuronSize
	 *            The number of elements to allocate on each neuron.
