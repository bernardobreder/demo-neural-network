...
public class NEATMutateAddLink extends NEATMutation {
	...
	public void performOperation(Random rnd, Genome[] parents, int parentIndex, Genome[] offspring,
		...
		while ((countTrysToAddLink--) > 0) {
			final NEATNeuronGene neuron1 = this.chooseRandomNeuron(target, true);
			final NEATNeuronGene neuron2 = this.chooseRandomNeuron(target, false);
			...
			if (!this.isDuplicateLink(target, neuron1.getId(), neuron2.getId())
					&& (neuron2.getNeuronType() != NEATNeuronType.Bias)
					&& (neuron2.getNeuronType() != NEATNeuronType.Input)) {

				if (((NEATPopulation) this.getOwner().getPopulation()).getActivationCycles() != 1
