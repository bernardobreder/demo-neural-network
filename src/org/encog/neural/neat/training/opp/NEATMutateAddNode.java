...
public class NEATMutateAddNode extends NEATMutation {
	...
	public void performOperation(final Random rnd, final Genome[] parents, final int parentIndex,
		...
		final NEATGenome target = this.obtainGenome(parents, parentIndex, offspring, offspringIndex);
		...
		final int sizeBias = ((NEATGenome) parents[0]).getInputCount() + ((NEATGenome) parents[0]).getOutputCount()
				+ 10;
		...
		while ((countTrysToFindOldLink--) > 0) {
			...
			if ((link.isEnabled()) && (target.getNeuronsChromosome().get(this.getElementPos(target, fromNeuron))
		...
		final NEATInnovation innovation = ((NEATPopulation) this.getOwner().getPopulation()).getInnovations()
				.findInnovationSplit(from, to);

		// add the splitting neuron
		final ActivationFunction af = ((NEATPopulation) this.getOwner().getPopulation()).getActivationFunctions()
				.pick(new Random());

		target.getNeuronsChromosome().add(
				new NEATNeuronGene(NEATNeuronType.Hidden, af, innovation.getNeuronID(), innovation.getInnovationID()));
