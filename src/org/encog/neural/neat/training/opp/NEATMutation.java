...
public abstract class NEATMutation implements EvolutionaryOperator {
	 ...
	 */
	public NEATNeuronGene chooseRandomNeuron(final NEATGenome target, final boolean choosingFrom) {
		int start;
		...
		// neurons
		if (!choosingFrom) {
			final int ac = ((NEATPopulation) target.getPopulation()).getActivationCycles();
			if (ac == 1) {
		...
		final NEATNeuronGene neuronGene = target.getNeuronsChromosome().get(neuronPos);
		return neuronGene;
	 ...
	 */
	public void createLink(final NEATGenome target, final long neuron1ID, final long neuron2ID, final double weight) {

		// first, does this link exist? (and if so, hopefully disabled,
		// otherwise we have a problem)
		for (final NEATLinkGene linkGene : target.getLinksChromosome()) {
			if ((linkGene.getFromNeuronID() == neuron1ID) && (linkGene.getToNeuronID() == neuron2ID)) {
		...
		final NEATLinkGene linkGene = new NEATLinkGene(neuron1ID, neuron2ID, true, innovation.getInnovationID(),
				weight);
	...
	public int getElementPos(final NEATGenome target, final long neuronID) {
		...
		for (int i = 0; i < target.getNeuronsChromosome().size(); i++) {
			final NEATNeuronGene neuronGene = target.getNeuronsChromosome().get(i);
	 ...
	 */
	public boolean isDuplicateLink(final NEATGenome target, final long fromNeuronID, final long toNeuronID) {
		for (final NEATLinkGene linkGene : target.getLinksChromosome()) {
			if ((linkGene.isEnabled()) && (linkGene.getFromNeuronID() == fromNeuronID)
	...
	public boolean isNeuronNeeded(final NEATGenome target, final long neuronID) {
		...
		for (final NEATNeuronGene gene : target.getNeuronsChromosome()) {
			if (gene.getId() == neuronID) {
				...
				if ((neuron.getNeuronType() == NEATNeuronType.Input) || (neuron.getNeuronType() == NEATNeuronType.Bias)
	 ...
	 */
	public NEATGenome obtainGenome(final Genome[] parents, final int parentIndex, final Genome[] offspring,
