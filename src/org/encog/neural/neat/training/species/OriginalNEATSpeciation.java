...
public class OriginalNEATSpeciation extends ThresholdSpeciation {
	 ...
	 */
	@Override
	public double getCompatibilityScore(final Genome gen1, final Genome gen2) {
		...
		while ((g1 < genome1Size) || (g2 < genome2Size)) {
			...
			if (id1 == id2) {
				...
				weightDifference += Math.abs(genome1.getLinksChromosome().get(g1).getWeight()
		...
		final double score = ((this.constExcess * numExcess) / n) + ((this.constDisjoint * numDisjoint) / n)
