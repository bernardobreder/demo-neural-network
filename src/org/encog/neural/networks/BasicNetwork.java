 ...
 *
 */
public class BasicNetwork extends BasicML implements ContainsFlat, MLContext, MLRegression, MLEncodable, MLResettable,
		MLClassification, MLError, MLFactory {
	 ...
	 *            The value to add.
	 */
	public void addWeight(final int fromLayer, final int fromNeuron, final int toNeuron, final double value) {
		final double old = this.getWeight(fromLayer, fromNeuron, toNeuron);
	...
	public String dumpWeightsVerbose() {
		...
		for (int layer = 0; layer < this.getLayerCount() - 1; layer++) {
			...
			for (int fromIdx = 0; fromIdx < this.getLayerNeuronCount(layer) + bias; fromIdx++) {
				for (int toIdx = 0; toIdx < this.getLayerNeuronCount(layer + 1); toIdx++) {
					...
					result.append(type1 + "-->" + type2 + toIdx + " : " + this.getWeight(layer, fromIdx, toIdx) + "\n");
				}
			}
		}
	 ...
	 */
	public void enableConnection(final int fromLayer, final int fromNeuron, final int toNeuron, final boolean enable) {
		...
		if (enable) {
			...
			if (Math.abs(value) < this.structure.getConnectionLimit()) {
				this.setWeight(fromLayer, fromNeuron, toNeuron, RangeRandomizer.randomize(-1, 1));
			}
		} else {
			if (!this.structure.isConnectionLimited()) {
	...
	public ActivationFunction getActivation(final int layer) {
		this.structure.requireFlat();
		final int layerNumber = this.getLayerCount() - layer - 1;
	...
	public double getLayerBiasActivation(final int l) {
		...
		this.structure.requireFlat();
		final int layerNumber = this.getLayerCount() - l - 1;

		final int layerOutputIndex = this.structure.getFlat().getLayerIndex()[layerNumber];
		final int count = this.structure.getFlat().getLayerCounts()[layerNumber];
	 ...
	 */
	public double getLayerOutput(final int layer, final int neuronNumber) {
		this.structure.requireFlat();
		final int layerNumber = this.getLayerCount() - layer - 1;
		final int index = this.structure.getFlat().getLayerIndex()[layerNumber] + neuronNumber;
		final double[] output = this.structure.getFlat().getLayerOutput();
		if (index >= output.length) {
			throw new NeuralNetworkError(
					"The layer index: " + index + " specifies an output index larger than the network has.");
		}
		return output[index];
	}
	 ...
	 */
	public boolean isConnected(final int layer, final int fromNeuron, final int toNeuron) {
	...
	public boolean isLayerBiased(final int l) {
		this.structure.requireFlat();
		final int layerNumber = this.getLayerCount() - l - 1;
		return this.structure.getFlat().getLayerCounts()[layerNumber] != this.structure.getFlat()
				.getLayerFeedCounts()[layerNumber];
	}
	...
	private Randomizer getRandomizer() {
		...
		for (int i = 0; i < this.getLayerCount(); i++) {
			ActivationFunction af = this.getActivation(i);
			if (af.getClass() != ActivationSigmoid.class && af.getClass() != ActivationTANH.class
					&& af.getClass() != ActivationElliott.class && af.getClass() != ActivationElliottSymmetric.class) {
				useNWR = false;
			}
		}
	 ...
	 *            The to value.
	 */
	public void setWeight(final int fromLayer, final int fromNeuron, final int toNeuron, final double value) {
		this.structure.requireFlat();
		final int fromLayerNumber = this.getLayerCount() - fromLayer - 1;
		...
		final int weightBaseIndex = this.structure.getFlat().getWeightIndex()[toLayerNumber];
		final int count = this.structure.getFlat().getLayerCounts()[fromLayerNumber];
	...
	public void validateNeuron(final int targetLayer, final int neuron) {
		...
		this.getFlat().validateNeuron(targetLayer, neuron);
	}
	...
	public int winner(final MLData input) {
		final MLData output = this.compute(input);
	...
	public String getFactoryArchitecture() {
		...
		for (int currentLayer = 0; currentLayer < this.getLayerCount(); currentLayer++) {
			...
			if (currentLayer > 0 && this.getActivation(currentLayer) != null) {
				ActivationFunction activationFunction = this.getActivation(currentLayer);
