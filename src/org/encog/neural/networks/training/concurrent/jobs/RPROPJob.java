...
public class RPROPJob extends TrainingJob {
	 ...
	 */
	public RPROPJob(final BasicNetwork network, final MLDataSet training, final boolean loadToMemory) {
	...
	public void createTrainer(final boolean singleThreaded) {
		final Propagation train = new ResilientPropagation(this.getNetwork(), this.getTraining(),
				this.getInitialUpdate(), this.getMaxStep());
		...
		for (final Strategy strategy : this.getStrategies()) {
