 ...
 * of gradient descent (similar to what is used by backpropagation. The lambda
 * parameter determines the degree to which GNA and Gradient Descent are used. A
 * lower lambda results in heavier use of GNA, whereas a higher lambda results
 * in a heavier use of gradient descent.
 *
 * Each iteration starts with a low lambda that builds if the improvement to the
 * neural network is not desirable. At some point the lambda is high enough that
 * the training method reverts totally to gradient descent.
 *
 ...
 *
 * References: http://www.heatonresearch.com/wiki/LMA
 * http://en.wikipedia.org/wiki/Levenberg%E2%80%93Marquardt_algorithm
 * http://en.wikipedia.org/wiki/Finite_difference_method
 * http://crsouza.blogspot.com/2009/11/neural-network-learning-by-levenberg_18.
 * html http://mathworld.wolfram.com/FiniteDifference.html
...
public class LevenbergMarquardtTraining extends BasicTraining implements MultiThreadable {
	 ...
	 */
	public LevenbergMarquardtTraining(final BasicNetwork network, final MLDataSet training) {
	 ...
	 *            Utility class to compute the Hessian.
	 */
	public LevenbergMarquardtTraining(final BasicNetwork network, final MLDataSet training, final ComputeHessian h) {
		...
		final BasicMLData input = new BasicMLData(this.indexableTraining.getInputSize());
		final BasicMLData ideal = new BasicMLData(this.indexableTraining.getIdealSize());
	...
	public void setThreadCount(int numThreads) {
		if (this.hessian instanceof MultiThreadable) {
			...
			throw new TrainingError("The Hessian object in use(" + this.hessian.getClass().toString()
