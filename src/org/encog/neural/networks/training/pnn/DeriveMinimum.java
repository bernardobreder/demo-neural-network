...
public class DeriveMinimum {
	 ...
	 */
	public double calculate(final int maxIterations, final double maxError, final double eps, final double tol,
			final CalculationCriteria network, final int n, final double[] x, final double ystart, final double[] base,
			final double[] direc, final double[] g, final double[] h, final double[] deriv2) {
		...
		for (int iteration = 0; iteration < maxIterations; iteration++) {
			...
			if (convergenceCounter > 0) {
				fbest = globalMinimum.brentmin(20, maxError, eps, 1.e-7, network, globalMinimum.getY2());
			} else {
				fbest = globalMinimum.brentmin(10, maxError, 1.e-6, 1.e-5, network, globalMinimum.getY2());
			}
