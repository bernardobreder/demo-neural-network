...
public class GlobalMinimumSearch {
	 ...
	 */
	public double brentmin(final int maxIterations, final double maxError, final double eps, final double tol,
		...
		for (int iter = 0; iter < maxIterations; iter++) {
			...
			if (Math.abs(prevdist) > tol1) {
				...
				final double numer = (xbest - x3rdBest) * t2 - (xbest - x2ndBest) * t1;
			...
			if (frecent <= fbest) { // If we improved...
				...
				if ((frecent <= fsecbest) || (x2ndBest == xbest)) {
					...
					fthirdbest = fsecbest;
					fsecbest = frecent;
				} else if ((frecent <= fthirdbest) || (x3rdBest == xbest) || (x3rdBest == x2ndBest)) {
					x3rdBest = xrecent;
					fthirdbest = frecent;
				}
			}
	 ...
	 */
	public void findBestRange(final double low, final double high, int numberOfPoints, final boolean useLog,
			final double minError, final CalculationCriteria network) {
		int i, ibest;
