...
public class TrainBasicPNN extends BasicTraining implements CalculationCriteria {
	...
	public double calcErrorWithMultipleSigma(final double[] x, final double[] der1, final double[] der2,
		...
		err = this.calculateError(this.network.getSamples(), true);
	...
	public double calculateError(final MLDataSet training, final boolean deriv) {
		...
		for (int r = 0; r < training.getRecordCount(); r++) {
			...
			if (this.network.getOutputMode() == PNNOutputMode.Unsupervised) {
				if (deriv) {
					final MLData output = this.computeDeriv(input, target);
				...
				if (deriv) {
					output = this.computeDeriv(input, pair.getIdeal());
				} else {
		...
		for (ivar = 0; ivar < this.network.getInputCount(); ivar++) {
			...
			for (outvar = 0; outvar < this.network.getOutputCount(); outvar++) {
				if ((this.network.getOutputMode() == PNNOutputMode.Classification)
					...
					this.v[outvar * this.network.getInputCount() + ivar] *= this.network.getPriors()[outvar]
							/ this.network.getCountPer()[outvar];
					this.w[outvar * this.network.getInputCount() + ivar] *= this.network.getPriors()[outvar]
							/ this.network.getCountPer()[outvar];
				}
	...
	public void iteration() {
		...
		final double d = dermin.calculate(32767, this.maxError, 1.e-8, this.minImprovement, this,
				this.network.getInputCount(), x, globalMinimum.getY2(), base, direc, g, h, dwk2);
