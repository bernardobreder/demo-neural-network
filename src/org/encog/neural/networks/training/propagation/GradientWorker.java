...
public class GradientWorker implements EngineTask {
	 ...
	 */
	public GradientWorker(final FlatNetwork theNetwork, final GradientWorkerOwner theOwner, final MLDataSet theTraining,
			final int theLow, final int theHigh, final double[] flatSpot, ErrorFunction ef) {
	...
	public void process(final MLDataPair pair) {
		...
		this.errorFunction.calculateError(this.network.getActivationFunctions()[0], this.layerSums, this.layerOutput,
				pair.getIdeal().getData(), this.actual, this.layerDelta, this.flatSpot[0], pair.getSignificance());

		// Apply regularization, if requested.
		if (this.owner.getL1() > Encog.DEFAULT_DOUBLE_EQUAL || this.owner.getL1() > Encog.DEFAULT_DOUBLE_EQUAL) {
	...
	public void layerRegularizationPenalty(final int fromLayer, final double[] l) {
		...
		final int toCount = this.network.getLayerNeuronCount(fromLayer + 1);
