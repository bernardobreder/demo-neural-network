...
public abstract class Propagation extends BasicTraining
		implements Train, MultiThreadable, BatchSize, GradientWorkerOwner {
		...
		try {
			for (int i = 0; i < count; i++) {
				...
				for (final GradientWorker worker : this.workers) {
					EngineArray.arrayCopy(this.currentFlatNetwork.getWeights(), 0, worker.getWeights(), 0,
				...
				EncogLogging.log(EncogLogging.LEVEL_INFO, "Training iteration done, error: " + this.getError());
	...
	private void copyContexts() {
		...
		for (int i = 0; i < (this.workers.length - 1); i++) {
			...
			final double[] dst = this.workers[i + 1].getNetwork().getLayerOutput();
			EngineArray.arrayCopy(src, dst);
		}

		// copy the contexts from the final group to the real network
		EngineArray.arrayCopy(this.workers[this.workers.length - 1].getNetwork().getLayerOutput(),
				this.currentFlatNetwork.getLayerOutput());
	}
	...
	private void init() {
		...
		if (this.shouldFixFlatSpot) {
			for (int i = 0; i < this.currentFlatNetwork.getActivationFunctions().length; i++) {
				final ActivationFunction af = this.currentFlatNetwork.getActivationFunctions()[i];
		...
		final DetermineWorkload determine = new DetermineWorkload(this.numThreads,
		...
		for (final IntRange r : determine.calculateWorkers()) {
			this.workers[index++] = new GradientWorker(this.currentFlatNetwork.clone(), this,
					this.indexable.openAdditional(), r.getLow(), r.getHigh(), this.flatSpot, this.ef);
		}
	 ...
	 */
	@Override
	public void report(final double[] gradients, final double error, final Throwable ex) {
		synchronized (this) {
			if (ex == null) {
	...
	/**
	 * Update a weight using dropout, the means by which weights are updated
	 * vary depending on the training.
	 *
	 * @param gradients
	 *            The gradients.
	 * @param lastGradient
	 ...
	 * @return The update value.
	 */
	public abstract double updateWeight(double[] gradients, double[] lastGradient, int index, double dropoutRate);
	...
	/**
	 * @param l1
	 *            How much to apply l1 regularization penalty, 0 (default) for
	 *            none.
	 */
	...
	/**
	 * @param l2
	 *            How much to apply l2 regularization penalty, 0 (default) for
	 *            none.
	 */
