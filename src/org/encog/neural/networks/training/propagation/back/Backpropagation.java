...
public class Backpropagation extends Propagation implements Momentum, LearningRate {
	 ...
	 */
	public Backpropagation(final ContainsFlat network, final MLDataSet training, final double theLearnRate,
	...
	public double updateWeight(final double[] gradients, final double[] lastGradient, final int index) {
		final double delta = (gradients[index] * this.learningRate) + (this.lastDelta[index] * this.momentum);
	...
	public double updateWeight(final double[] gradients, final double[] lastGradient, final int index,
		...
		if (this.nesterovUpdate) {
			return this.updateWeightNesterov(gradients, lastGradient, index, dropoutRate);
		} else {
			return this.updateWeightNormal(gradients, lastGradient, index, dropoutRate);
		}
	}
	 ...
	 * @return The weight delta.
	 */
	private double updateWeightNormal(final double[] gradients, final double[] lastGradient, final int index,
			double dropoutRate) {
	 ...
	 * @return The weight delta.
	 */
	private double updateWeightNesterov(final double[] gradients, final double[] lastGradient, final int index,
			double dropoutRate) {
		...
		this.lastDelta[index] = (this.momentum * prevNesterov) + (this.gradients[index] * this.learningRate);
