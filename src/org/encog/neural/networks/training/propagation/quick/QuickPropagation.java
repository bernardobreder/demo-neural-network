 ...
 * was introduced in a paper:
 *
 * An Empirical Study of Learning Speed in Back-Propagation Networks" (Scott E.
 * Fahlman, 1988)
 *
 *
...
public class QuickPropagation extends Propagation implements LearningRate {
	...
	/**
	 * This factor times the current weight is added to the slope at the start
	 * of each output epoch. Keeps weights from growing too big.
	 */
	...
	/**
	 * Controls the amount of linear gradient descent to use in updating output
	 * weights.
	 */
	private double outputEpsilon = 0.35;

	/**
	 * Used in computing whether the proposed step is too large. Related to
	 * learningRate.
	 */
	 ...
	 *            The learning rate. 2 is a good suggestion as a learning rate
	 *            to start with. If it fails to converge, then drop it. Just
	 *            like backprop, except QPROP can take higher learning rates.
	 */
	public QuickPropagation(final ContainsFlat network, final MLDataSet training, final double theLearningRate) {
	...
	public void resume(final TrainingContinuation state) {
		...
		final double[] lastGradient = (double[]) state.get(QuickPropagation.LAST_GRADIENTS);
	...
	/**
	 * @param theOutputEpsilon
	 *            the outputEpsilon to set
	 */
	...
	public void initOthers() {
		this.eps = this.outputEpsilon / this.getTraining().getRecordCount();
