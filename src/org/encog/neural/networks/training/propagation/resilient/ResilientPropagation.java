 ...
 *
 * It is also important to note that RPROP does not work well with online
 * training. You should always use a batch size bigger than one. Typically the
 * larger the better. By default a batch size of zero is used, zero means to
 * include the entire training set in the batch.
 *
...
public class ResilientPropagation extends Propagation {
	 ...
	 */
	public ResilientPropagation(final ContainsFlat network, final MLDataSet training) {
	 ...
	 */
	public ResilientPropagation(final ContainsFlat network, final MLDataSet training, final double initialUpdate,
	...
	public boolean isValidResume(final TrainingContinuation state) {
		if (!state.getContents().containsKey(ResilientPropagation.LAST_GRADIENTS)
				|| !state.getContents().containsKey(ResilientPropagation.UPDATE_VALUES)) {
			return false;
		}
		...
		final double[] d = (double[]) state.get(ResilientPropagation.LAST_GRADIENTS);
	...
	public TrainingContinuation pause() {
		...
		result.set(ResilientPropagation.LAST_GRADIENTS, this.getLastGradient());
		result.set(ResilientPropagation.UPDATE_VALUES, this.getUpdateValues());
	...
	public void resume(final TrainingContinuation state) {
		...
		final double[] lastGradient = (double[]) state.get(ResilientPropagation.LAST_GRADIENTS);
		final double[] updateValues = (double[]) state.get(ResilientPropagation.UPDATE_VALUES);
	...
	/**
	 * Set the type of RPROP to use. The default is RPROPp (RPROP+), or classic
	 * RPROP.
	 *
	 * @param t
	 *            The type.
	...
	public void setRPROPType(RPROPType t) {
		ResilientPropagation.rpropType = t;
	}
	...
	public RPROPType getRPROPType() {
		return ResilientPropagation.rpropType;
	}
	...
	public double updateWeight(final double[] gradients, final double[] lastGradient, final int index) {
		...
		switch (ResilientPropagation.rpropType) {
		case RPROPp:
		...
		case ARPROP:
			...
			break;
		default:
			throw new TrainingError("Unknown RPROP type: " + ResilientPropagation.rpropType);
		}
	...
	public double updateJacobiWeight(final double[] gradients, final double[] lastGradient, final int index) {
