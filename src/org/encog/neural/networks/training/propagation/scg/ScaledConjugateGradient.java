...
public class ScaledConjugateGradient extends Propagation {
	 ...
	 *            The training data.
	 */
	public ScaledConjugateGradient(final ContainsFlat network, final MLDataSet training) {
	...
	public void iteration() {
		...
		if (this.success) {
			...
			final double sigma = ScaledConjugateGradient.FIRST_SIGMA / Math.sqrt(this.magP);
			...
			this.delta = 0;
			for (int i = 0; i < numWeights; ++i) {
				final double step = (this.gradients[i] - this.oldGradient[i]) / sigma;
		...
		final double gdelta = 2 * this.delta * (this.oldError - this.getError()) / (mu * mu);
		...
		if (gdelta >= 0) {
			...
			if (this.k >= numWeights) {
				...
				final double beta = (EngineArray.vectorProduct(this.r, this.r) - rsum) / mu;
