 ...
 * optimisation (PSO).
 *
 * Contributed by: Geoffroy Noel https://github.com/goffer-looney
 *
 * References: James Kennedy and Russell C. Eberhart, Particle swarm
...
public class NeuralPSO extends BasicTraining {
	 ...
	 *
	 * @param network
	 *            an initialised Encog network. The networks in the swarm will
	 *            be created with the same topology as this network.
	 * @param randomizer
	 *            any type of Encog network weight initialisation object.
	 * @param calculateScore
	 *            any type of Encog network scoring/fitness object.
	 * @param populationSize
	 *            the swarm size.
	 */
	public NeuralPSO(final BasicNetwork network, final Randomizer randomizer, final CalculateScore calculateScore,
	 ...
	 *
	 * @param network
	 *            The network to train. an initialised Encog network. The
	 *            networks in the swarm will be created with the same topology
	 *            as this network.
	 * @param trainingSet
	 *            The training set.
	 */
	...
	/**
	 * Initialise the particle positions and velocities, personal and global
	 * bests. Only does this if they have not yet been initialised.
	 */
	void initPopulation() {
	 ...
	 *
	 * @param init
	 *            true if this is an initialisation iteration.
	 */
	 ...
	 *
	 * @param particleIndex
	 *            index of the particle in the swarm
	 * @param init
	 *            if true, the position and velocity will be initialised.
	 */
	 ...
	 *            the particle current position vector
	 */
	protected void updateVelocity(int particleIndex, double[] particlePosition) {
		...
		if (i != this.m_bestVectorIndex) {
			this.m_va.copy(vtmp,
					this.m_pseudoAsynchronousUpdate ? this.m_bestVectors[this.m_bestVectorIndex] : this.m_bestVector);
			this.m_va.sub(vtmp, particlePosition);
	...
	protected void updatePersonalBestPosition(int particleIndex, double[] particlePosition) {
		...
		if ((this.m_bestErrors[particleIndex] == 0) || this.isScoreBetter(score, this.m_bestErrors[particleIndex])) {
	...
	protected void updateGlobalBestPosition() {
		...
		for (int i = 0; i < this.m_populationSize; i++) {
			if ((this.m_bestVectorIndex == -1) || this.isScoreBetter(this.m_bestErrors[i], currentBestError)) {
	...
	boolean isScoreBetter(double score1, double score2) {
		return ((this.m_calculateScore.shouldMinimize() && (score1 < score2))
				|| ((!this.m_calculateScore.shouldMinimize()) && (score1 > score2)));
	}
	 ...
	 *            index of the network in the swarm
	 * @param state
	 *            an array of weights and biases
	 */
	 ...
	 *
	 * @param maxPosition
	 *            maximum value for a component (Xmax)
	 */
	...
	public String getDescription() {
		return String.format("pop = %d, w = %.2f, c1 = %.2f, c2 = %.2f, Xmax = %.2f, Vmax = %.2f",
				this.m_populationSize, this.m_inertiaWeight, this.m_c1, this.m_c2, this.m_maxPosition,
				this.m_maxVelocity);
	}
	...
	/**
	 * Keep a reference to the passed population of networks. This population is
	 * not copied, it will evolve during training.
	 *
	 * @param initialPopulation
	 *            the initial population of networks.
	 */
