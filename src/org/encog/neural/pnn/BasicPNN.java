 ...
 *
 * by Timothy Masters, PhD (<a href="http://www.timothymasters.info/"></a>) John
 * Wiley and Sons Inc (Computers); April 3, 1995, ISBN: 0471105880
 */
public class BasicPNN extends AbstractPNN implements MLRegression, MLError, MLClassification {
	...
	public MLData compute(final MLData input) {
		...
		final double[] out = new double[this.getOutputCount()];
		...
		for (final MLDataPair pair : this.samples) {
			...
			if (this.getOutputMode() == PNNOutputMode.Classification) {
				...
				out[pop] += dist;
			} else if (this.getOutputMode() == PNNOutputMode.Unsupervised) {
				for (int i = 0; i < this.getInputCount(); i++) {
					out[i] += dist * pair.getInput().getData(i);
				}
				psum += dist;
			} else if (this.getOutputMode() == PNNOutputMode.Regression) {
