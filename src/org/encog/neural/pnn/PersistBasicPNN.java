...
public class PersistBasicPNN implements EncogPersistor {
	...
	public Object read(final InputStream is) {
		...
		while ((section = in.readNextSection()) != null) {
			...
			if (section.getSectionName().equals("PNN") && section.getSubSectionName().equals("NETWORK")) {
				...
				kernel = PersistBasicPNN.stringToKernel(params.get(PersistConst.KERNEL));
				outmodel = PersistBasicPNN.stringToOutputMode(params.get(PersistBasicPNN.PROPERTY_outputMode));
				error = EncogFileSection.parseDouble(params, PersistConst.ERROR);
				sigma = section.parseDoubleArray(params, PersistConst.SIGMA);
			}
	...
	public void save(final OutputStream os, final Object obj) {
		...
		out.writeProperty(PersistConst.KERNEL, PersistBasicPNN.kernelToString(pnn.getKernel()));
		out.writeProperty(PersistConst.OUTPUT_COUNT, pnn.getOutputCount());
		out.writeProperty(PersistBasicPNN.PROPERTY_outputMode, PersistBasicPNN.outputModeToString(pnn.getOutputMode()));
