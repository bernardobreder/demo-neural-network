...
public class PruneIncremental extends ConcurrentJob {
	 ...
	 */
	private final List<HiddenLayerParams> hidden = new ArrayList<>();
	 ...
	 */
	public PruneIncremental(final MLDataSet training, final NeuralNetworkPattern pattern, final int iterations,
			final int weightTries, final int numTopResults, final StatusReportable report) {
		super(report);
		if (numTopResults < 1) {
	...
	public void performJobUnit(final JobUnitContext context) {
		...
		for (int z = 0; z < this.weightTries; z++) {
			network.reset();
			final Propagation train = new ResilientPropagation(network, useTraining);
			final StopTrainingStrategy strat = new StopTrainingStrategy(0.001, 5);
		...
		if (!this.getShouldStop()) {
			...
			this.reportStatus(context, "Current: " + PruneIncremental.networkToString(network) + "; Best: "
	...
	public void process() {
		...
		if (this.hidden.size() == 0) {
			throw new EncogError(
					"To calculate the optimal hidden size, at least " + "one hidden layer must be defined.");
		}
		...
		if (this.hiddenCounts[0] == 0) {
			throw new EncogError("To calculate the optimal hidden size, at least "
	...
	public Object requestNextTask() {
		...
		final BasicNetwork network = this.generateNetwork();
	 ...
	 */
	private synchronized void updateBest(final BasicNetwork network, final double error) {
		this.high = Math.max(this.high, error);
		this.low = Math.min(this.low, error);
		...
		for (int i = 0; i < this.topNetworks.length; i++) {
			if (this.topNetworks[i] == null) {
				...
				if ((selectedIndex == -1) || (this.topErrors[selectedIndex] < this.topErrors[i])) {
					selectedIndex = i;
				}
			}
		}
		...
		for (final BasicNetwork n : this.topNetworks) {
			...
			if (choice == null) {
				...
				if (n.getStructure().calculateSize() < choice.getStructure().calculateSize()) {
					choice = n;
				}
			}
		}
