...
public class PruneSelective {
	...
	private void decreaseNeuronCount(final int layer, final int neuronCount) {
		...
		final int lostNeuronCount = this.network.getLayerNeuronCount(layer) - neuronCount;
		final int[] lostNeuron = this.findWeakestNeurons(layer, lostNeuronCount);
	...
	public double determineNeuronSignificance(final int layer, final int neuron) {
		...
		if (layer > 0) {
			...
			final int prevCount = this.network.getLayerTotalNeuronCount(prevLayer);
	...
	private int[] findWeakestNeurons(final int layer, final int count) {
		...
		for (int i = count; i < this.network.getLayerNeuronCount(layer); i++) {
			final double significance = this.determineNeuronSignificance(layer, i);
	...
	private void increaseNeuronCount(final int targetLayer, final int neuronCount) {
		...
		final int oldNeuronCount = this.network.getLayerNeuronCount(targetLayer);
		...
		for (int fromLayer = flat.getLayerCounts().length - 2; fromLayer >= 0; fromLayer--) {
			final int fromNeuronCount = this.network.getLayerTotalNeuronCount(fromLayer);
			final int toNeuronCount = this.network.getLayerNeuronCount(fromLayer + 1);
	 ...
	 *            The target neuron.
	 */
	public void randomizeNeuron(final double low, final double high, final int targetLayer, final int neuron) {
	 ...
	 */
	private void randomizeNeuron(final int targetLayer, final int neuron, final boolean useRange, final double low,
			final double high, final boolean usePercent, final double percent) {
	 ...
	 */
	public void stimulateNeuron(final double percent, final int targetLayer, final int neuron) {

		this.randomizeNeuron(targetLayer, neuron, false, 0, 0, true, percent);
	}
	 ...
	 *            The percent to stimulate by.
	 */
	public void stimulateWeakNeurons(final int layer, final int count, final double percent) {
		final int[] weak = this.findWeakestNeurons(layer, count);
