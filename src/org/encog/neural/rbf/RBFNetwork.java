...
public class RBFNetwork extends BasicML implements MLError, MLRegression, ContainsFlat, MLResettable, MLEncodable {
	 ...
	 *            The RBF type.
	 */
	public RBFNetwork(final int inputCount, final int hiddenCount, final int outputCount, final RBFEnum t) {
	 ...
	 *            The RBF type.
	 */
	public RBFNetwork(final int inputCount, final int outputCount, final RadialBasisFunction[] rbf) {
		this.flat = new FlatNetworkRBF(inputCount, rbf.length, outputCount, rbf);
	...
	public MLData compute(final MLData input) {
		final MLData output = new BasicMLData(this.getOutputCount());
	 ...
	 */
	public void setRBFCentersAndWidths(final double[][] centers, final double[] widths, final RBFEnum t) {
		for (int i = 0; i < this.flat.getRBF().length; i++) {
	 ...
	 */
	public void setRBFCentersAndWidthsEqualSpacing(final double minPosition, final double maxPosition, final RBFEnum t,
		...
		for (int i = 0; i < totalNumHiddenNeurons; i++) {
			...
			for (int j = dimensions; j > 0; j--) {
				...
				centers[i][j
						- 1] = ((int) (temp / Math.pow(sideLength, j - 1)) * (disMinMaxPosition / (sideLength - 1)))
								+ minPosition;
	 ...
	 *            The width.
	 */
	public void setRBFFunction(final int index, final RBFEnum t, final double[] centers, final double width) {
