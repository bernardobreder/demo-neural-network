...
public class SVD {
	public static double svdfit(double[][] x, double[][] y, double[][] a, RadialBasisFunction[] funcs) {
		...
		// Fill input matrix with values based on fitting functions and input
		// coordinates
		...
		// Check for w values that are close to zero and replace them with zeros
		// such that they are ignored in backsub
		wmax = 0;
		for (j = 0; j < funcs.length; j++) {
			if (w[j] > wmax) {
				wmax = w[j];
