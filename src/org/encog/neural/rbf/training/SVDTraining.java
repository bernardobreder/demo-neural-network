...
public class SVDTraining extends BasicTraining {
	...
	public void flatToMatrix(final double[] flat, final int start, final double[][] matrix) {
	...
	public void iteration() {
		...
		final ObjectPair<double[][], double[][]> data = TrainingSetUtil.trainingToArray(this.getTraining());

		final double[][] matrix = new double[length][this.network.getOutputCount()];
	 ...
	 *            WHere to start.
	 */
	public void matrixToFlat(final double[][] matrix, final double[] flat, final int start) {
		final int rows = matrix.length;
		final int cols = matrix[0].length;
