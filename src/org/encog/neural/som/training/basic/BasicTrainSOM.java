...
public class BasicTrainSOM extends BasicTraining implements LearningRate {
	 ...
	 */
	public BasicTrainSOM(final SOM network, final double learningRate, final MLDataSet training,
	 ...
	 */
	private void copyInputPattern(final Matrix matrix, final int outputNeuron, final MLData input) {
	 ...
	 */
	private double determineNewWeight(final double weight, final double input, final int currentNeuron, final int bmu) {

		final double newWeight = weight
				+ (this.neighborhood.function(currentNeuron, bmu) * this.learningRate * (input - weight));
		return newWeight;
	}
	 ...
	 */
	private boolean forceWinners(final Matrix matrix, final int[] won, final MLData leastRepresented) {

		double maxActivation = Double.MIN_VALUE;
		int maxActivationNeuron = -1;

		final MLData output = this.compute(this.network, leastRepresented);
	...
	public void iteration() {
		...
		for (final MLDataPair pair : this.getTraining()) {
			...
			if (this.forceWinner) {
				...
				final MLData output = this.compute(this.network, pair.getInput());
			...
			if (this.forceWinner) {
				...
				if (!this.forceWinners(this.network.getWeights(), won, leastRepresented)) {
	 ...
	 *            The ending radius.
	 */
	public void setAutoDecay(final int plannedIterations, final double startRate, final double endRate,
	 ...
	 */
	private void trainPattern(final Matrix matrix, final MLData input, final int current, final int bmu) {
		...
		for (int inputNeuron = 0; inputNeuron < this.inputNeuronCount; inputNeuron++) {
			...
			final double newWeight = this.determineNewWeight(currentWeight, inputValue, current, bmu);
	...
	/**
	 * Calculate the output of the SOM, for each output neuron. Typically, you
	 * will use the classify method instead of calling this method.
	 *
	 * @param input
	 *            The input pattern.
