...
public class BestMatchingUnit {
	...
	public int calculateBMU(final MLData input) {
		...
		if (input.size() > this.som.getInputCount()) {
			throw new NeuralNetworkError("Can't train SOM with input size of " + this.som.getInputCount()
		...
		for (int i = 0; i < this.som.getOutputCount(); i++) {
			final double distance = this.calculateEuclideanDistance(this.som.getWeights(), input, i);
	 ...
	 */
	public double calculateEuclideanDistance(final Matrix matrix, final MLData input, final int outputNeuron) {
		double result = 0;
	...
	/**
	 * @return What is the worst BMU distance so far, this becomes the error for
	 *         the entire SOM.
	 */
	...
	/**
	 * Reset the "worst distance" back to a minimum value. This should be called
