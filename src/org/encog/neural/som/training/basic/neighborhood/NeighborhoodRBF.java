...
public class NeighborhoodRBF implements NeighborhoodFunction {
	...
	/**
	 * The displacement of each dimension, when mapping the dimensions to a 1d
	 * array.
	 */
	private int[] displacement;

	/**
	 * Construct a 2d neighborhood function based on the sizes for the x and y
	 * dimensions.
	 *
	 * @param type
	 *            The RBF type to use.
	...
	public double function(final int currentNeuron, final int bestNeuron) {
		...
		final int[] vectorCurrent = this.translateCoordinates(currentNeuron);
		final int[] vectorBest = this.translateCoordinates(bestNeuron);
	...
	/**
	 * Translate the specified index into a set of multi-dimensional coordinates
	 * that represent the same index. This is how the multi-dimensional
	 * coordinates are translated into a one dimensional index for the input
	 * neurons.
	 *
	 * @param index
