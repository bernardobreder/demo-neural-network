...
public class HopfieldNetwork extends ThermalNetwork {
	...
	public void addPattern(final MLData pattern) {
		...
		if (pattern.size() != this.getNeuronCount()) {
			throw new NeuralNetworkError("Network with " + this.getNeuronCount()
					+ " neurons, cannot learn a pattern of size " + pattern.size());
		}
	...
	public void run() {
		...
		for (int toNeuron = 0; toNeuron < this.getNeuronCount(); toNeuron++) {
			double sum = 0;
			for (int fromNeuron = 0; fromNeuron < this.getNeuronCount(); fromNeuron++) {
