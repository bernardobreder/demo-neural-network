...
public class ReadTags {
	 ...
	 * <br/>
	 * )? If so, this will hold a string for that tag.
	 */
	...
	protected String parseAttributeName() {
		...
		if ("\"\'".indexOf(this.source.peek()) == -1) {
			...
			while (!Character.isWhitespace(this.source.peek()) && (this.source.peek() != '=')
					&& (this.source.peek() != '>') && (this.source.peek() != -1)) {
				final int ch = this.parseSpecialCharacter();
	...
	protected String parseString() {
		...
		if ("\"\'".indexOf(this.source.peek()) != -1) {
			...
			while (!Character.isWhitespace(this.source.peek()) && (this.source.peek() != -1)
					&& (this.source.peek() != '>')) {
	...
	protected void parseTag() {
		...
		while ((this.source.peek() != '>') && (this.source.peek() != -1)) {
			final String attributeName = this.parseAttributeName();
	...
	public int read() {
		...
		if (this.source.peek() == '<') {
			this.parseTag();
			if ((this.tag.getType() == Tag.Type.BEGIN) && (this.tag.getName().equalsIgnoreCase("script")
