...
public class EncogDirectoryPersistence {
	...
	public static Object loadObject(final InputStream is) {
		...
		final EncogPersistor p = PersistorRegistry.getInstance().getPersistor(name);
	 ...
	 *            The Object to save.
	 */
	public static void saveObject(final File filename, final Object obj) {
	...
	public static void saveObject(final OutputStream os, final Object obj) {
		try {
			final EncogPersistor p = PersistorRegistry.getInstance().getPersistor(obj.getClass());
			...
			pw.println("encog," + obj.getClass().getSimpleName() + ",java," + Encog.VERSION + "," + p.getFileVersion()
					+ "," + now.getTime());
			pw.flush();
			p.save(os, obj);
		} catch (final IOException ex) {
	...
	/**
	 * Get the type of an Encog object in an EG file, without the need to read
	 * the entire file.
	 *
	 * @param name
