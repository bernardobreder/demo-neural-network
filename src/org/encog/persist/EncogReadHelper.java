...
public class EncogReadHelper {
	...
	public EncogReadHelper(final InputStream is) {
		try {
			this.reader = new BufferedReader(new InputStreamReader(is, Encog.DEFAULT_ENCODING));
	...
	public final EncogFileSection readNextSection() {
		...
		try {
			String line;
			List<double[]> largeArrays = new ArrayList<>();
					...
					if (idx == -1) {
						...
						if (this.currentSectionName.length() < 1) {
							throw new PersistError(
									"Can't begin subsection when a section has not yet been defined: " + line);
						}
						...
						if (!newSection.equals(this.currentSectionName)) {
							throw new PersistError("Can't begin subsection " + line
									+ ", while we are still in the section: " + this.currentSectionName);
						}
	 ...
	 *
	 * @param line
	 *            The line containing the beginning of a large array.
