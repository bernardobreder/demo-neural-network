...
public class EncogWriteHelper {
	 ...
	 */
	public final void writeProperty(final String name, final ActivationFunction act) {
	...
	public final void writeProperty(final String name, final CSVFormat csvFormat) {
		String fmt;
		if ((csvFormat == CSVFormat.ENGLISH) || (csvFormat == CSVFormat.DECIMAL_POINT)) {
			fmt = "decpnt";
	...
	public final void writeProperty(final String name, final double value) {
		this.out.println(name + "=" + CSVFormat.EG_FORMAT.format(value, Encog.DEFAULT_PRECISION));
	}
		...
		if (d.length < 2048) {
			...
			while (index < d.length) {
				...
				for (int i = 0; (i < 2048) && (index < d.length); i++) {
					...
					this.out.print(CSVFormat.EG_FORMAT.format(d[index], Encog.DEFAULT_PRECISION));
					index++;
					first = false;
				}
				this.out.println();
