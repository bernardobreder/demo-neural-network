...
public class ImageMLDataSet extends BasicMLDataSet {
	 ...
	 *            The low value to normalize to.
	 */
	public ImageMLDataSet(final Downsample theDownsampler, final boolean theFindBounds, final double theHi,
			final double theLo) {
		...
		for (final MLDataPair pair : this) {
			if (!(pair.getInput() instanceof ImageMLData)) {
				throw new NeuralNetworkError(
						"Invalid class type found in ImageNeuralDataSet, only " + "ImageNeuralData items are allowed.");
			}
