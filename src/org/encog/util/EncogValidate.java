...
public final class EncogValidate {
	 ...
	 */
	public static void validateNetworkForTraining(final ContainsFlat network, final MLDataSet training) {
		...
		if ((training.getIdealSize() > 0) && (outputCount != training.getIdealSize())) {
			throw new NeuralNetworkError("The output layer size of " + outputCount
					+ " must match the training input size of " + training.getIdealSize() + ".");
		}
	}
