...
public class EncogBenchmark {
	...
	public String process() {
		...
		this.report.report(EncogBenchmark.STEPS, EncogBenchmark.STEPS, result.toString());
	...
	private void evalCPU() {
		...
		this.report.report(EncogBenchmark.STEPS, EncogBenchmark.STEP1, "CPU result: " + result);
		this.cpuScore = result;
	}

	private void evalMemory() {
		final BasicMLDataSet training = RandomTrainingFactory.generate(1000, 10000, 10, 10, -1, 1);
	...
	private void evalBinary() {
		...
		this.report.report(EncogBenchmark.STEPS, EncogBenchmark.STEP4,
				"Disk(binary) dataset, result: " + Format.formatInteger(iterations));
