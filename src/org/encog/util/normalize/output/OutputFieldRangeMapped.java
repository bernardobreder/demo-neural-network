...
public class OutputFieldRangeMapped extends BasicOutputField implements RequireTwoPass {
	 ...
	 *            The to map.
	 * @param min
	 *            The minimum that the value param can be.
	 * @param max
	 *            The maximum that the value param can be.
	 * @param hi
	 ...
	 */
	public static double calculate(final double value, final double min, final double max, final double hi,
			final double lo) {
	 ...
	 *            The high value.
	 */
	public OutputFieldRangeMapped(final InputField field, final double low, final double high) {
		this.field = field;
		this.low = low;
		this.high = high;
	}
	...
	public double calculate(final int subfield) {
		if (this.field.getMax() == this.field.getMin()) {
			...
			return ((this.field.getCurrentValue() - this.field.getMin()) / (this.field.getMax() - this.field.getMin()))
	...
	public double convertBack(final double data) {
		double result = ((this.field.getMin() - this.field.getMax()) * data - this.high * this.field.getMin()
				+ this.field.getMax() * this.low) / (this.low - this.high);
		return result;
	}
