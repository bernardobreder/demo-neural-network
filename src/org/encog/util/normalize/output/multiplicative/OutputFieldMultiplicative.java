 ...
 *
 * Multiplicative normalization works by calculating the vector length of the
 * input fields and dividing each by that value. This also presents a problem,
 * as the magnitude of the original fields is not used. For example,
 * multiplicative normalization would not distinguish between (-2,1,3) and
 * (-10,5,15). Both would result in the same output.
 *
 * @author jheaton
 */
public class OutputFieldMultiplicative extends OutputFieldGrouped {
	 ...
	 */
	public OutputFieldMultiplicative(final OutputFieldGroup group, final InputField field) {
		super(group, field);
