...
public class OutputOneOf extends BasicOutputField {
	 ...
	 */
	private final List<NominalItem> items = new ArrayList<>();
	...
	public OutputOneOf(InputFieldCSVText csv, final double trueValue, final double falseValue) {
		...
		for (String s : csv.getMappings().keySet()) {
			this.addItem(csv, csv.getMappings().get(s));
		}
	}
	 ...
	 * @param value
	 *            The value to calculate the high and low values off of.
	 */
	 ...
	 *
	 * @param inputField
	 *            The input field to base everything from.
	 * @param low
