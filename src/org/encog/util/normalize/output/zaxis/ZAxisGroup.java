 ...
 * OutputFieldZAxisSynthetic fields may belong to this group. For more
 * information see the OutputFieldZAxis class.
 *
 */
public class ZAxisGroup extends BasicOutputFieldGroup {
	...
	public void rowInit() {
		...
		for (final OutputFieldGrouped field : this.getGroupedFields()) {
			if (!(field instanceof OutputFieldZAxisSynthetic)) {
				if (field.getSourceField() != null) {
					value += (field.getSourceField().getCurrentValue() * field.getSourceField().getCurrentValue());
				}
			}
		}
		this.length = Math.sqrt(value);
		this.multiplier = 1.0 / Math.sqrt(this.getGroupedFields().size());
	}
