...
public class AnalystTestingUtility {
	...
	public AnalystTestingUtility(String theBaseDataFile) {
		System.gc();
		this.tempDir.clearContents();
		this.rawFile = this.tempDir.createFile("test.csv");
		FileUtil.copyResource(theBaseDataFile, this.rawFile);
		this.analystFile = this.tempDir.createFile("test.ega");
	...
	public void wizard(AnalystGoal goal, WizardMethodType methodType, boolean headers) {
		...
		wiz.wizard(this.rawFile, headers, this.format);

		this.encogAnalyst.save(this.analystFile);
		this.encogAnalyst.load(this.analystFile);
	}
	...
	public void report() {
		...
		report.produceReport(this.tempDir.createFile("report.html"));
	}

	public void validateDataField(int i, double max, double mean, double min, double sd, String name, boolean isClass,
			boolean isComplete, boolean isInteger, boolean isReal) {
	...
	public MLMethod obtainMethod() {
		...
		final String resourceID = prop.getPropertyString(ScriptProperties.ML_CONFIG_MACHINE_LEARNING_FILE);
		...
		if (!(method instanceof MLMethod)) {
			throw new AnalystError(
					"The object to be trained must be an instance of MLMethod. " + method.getClass().getSimpleName());
		}
	...
	private MLDataSet obtainTrainingSet() {
		final ScriptProperties prop = this.encogAnalyst.getScript().getProperties();
		final String trainingID = prop.getPropertyString(ScriptProperties.ML_CONFIG_TRAINING_FILE);
	...
	public double calculateError() {
		...
		MLDataSet data = this.obtainTrainingSet();
	...
	public void validateAnalystField(int i, double high, double low, double normHigh, double normLow, String name,
			int timeSlice, String action) {
		AnalystField af = this.encogAnalyst.getScript().getNormalize().getNormalizedFields().get(i);
