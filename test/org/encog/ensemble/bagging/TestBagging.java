...
public class TestBagging extends TestCase {
	...
	public void testBagging() {
		this.trainingData = XOR.createXORDataSet();
		XOR.testXORDataSet(this.trainingData);
		this.trainingData = new EnsembleDataSet(this.trainingData);
		assertEquals(1, this.trainingData.getIdealSize());
		assertEquals(2, this.trainingData.getInputSize());
		...
		ArrayList<Integer> middleLayers = new ArrayList<>();
		middleLayers.add(4);
		mlpFactory.setParameters(middleLayers, new ActivationSigmoid());
		MajorityVoting mv = new MajorityVoting();
		Bagging testBagging = new Bagging(this.numSplits, this.dataSetSize, mlpFactory, trainingStrategy, mv);
		testBagging.setTrainingData(this.trainingData);
		try {
			testBagging.train(1E-2, 1E-2, (EnsembleDataSet) this.trainingData);
		} catch (TrainingAborted e) {
			e.printStackTrace();
		}
		for (int j = 0; j < this.trainingData.size(); j++) {
			MLData input = this.trainingData.get(j).getInput();
			MLData result;
			try {
				result = testBagging.compute(input);
				MLData should = this.trainingData.get(j).getIdeal();
