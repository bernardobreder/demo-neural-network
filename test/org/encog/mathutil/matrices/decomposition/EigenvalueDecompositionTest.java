...
public class EigenvalueDecompositionTest {
	...
	private Matrix matrix1 = new Matrix(new double[][] { { 2.0, 5.1 }, { -0.5, 0.9 } });

	private Matrix matrix2 = new Matrix(new double[][] { { 2.0, 5.5 }, { 5.5, -1.0 } });

	@Test
	public void testEigenvalueDecomposition() {
		new EigenvalueDecomposition(this.matrix1);
	}

	@Test
	public void testGetD_asymentric() {
		EigenvalueDecomposition decomposition = new EigenvalueDecomposition(this.matrix1);
	...
	public void testGetD_symentric() {
		EigenvalueDecomposition decomposition = new EigenvalueDecomposition(this.matrix2);
		Matrix d = decomposition.getD();
		assertEquals(2, d.getCols());
		assertEquals(2, d.getRows());
	...
	public void testIsSymmetric() {
		assertTrue("An empty matrix should be symmetric", EigenvalueDecomposition.isSymmetric(new double[][] {}));
		assertFalse("An matrix with a row but no values should not be symmetric",
				EigenvalueDecomposition.isSymmetric(new double[][] { {} }));
		assertTrue("A singleton matrix should be symmetric",
				EigenvalueDecomposition.isSymmetric(new double[][] { { 0 } }));
		assertFalse("A malformed matrix should not be symmetric",
				EigenvalueDecomposition.isSymmetric(new double[][] { { 0, 1 } }));
		assertTrue("This simple matrix should be symmetric",
				EigenvalueDecomposition.isSymmetric(new double[][] { { 0, 1 }, { 1, 0 }, }));
		assertFalse("This simple matrix should not be symmetric",
				EigenvalueDecomposition.isSymmetric(new double[][] { { 0, 1 }, { 2, 0 }, }));
		assertTrue("This matrix should be symmetric", EigenvalueDecomposition
				.isSymmetric(new double[][] { { 0, 1, 3, 8 }, { 1, 5, 7, 1 }, { 3, 7, 9, 0 }, { 8, 1, 0, 6 }, }));
		assertFalse("This matrix should not be symmetric", EigenvalueDecomposition
				.isSymmetric(new double[][] { { 8, 1, 0, 6 }, { 3, 7, 9, 0 }, { 1, 5, 7, 1 }, { 0, 1, 3, 8 }, }));
	}
}
