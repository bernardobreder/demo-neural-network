...
public class TestNEAT extends TestCase {
	...
	public final File EGB_FILENAME = this.TEMP_DIR.createFile("encogtest.egb");

	public void testNEATBuffered() {
		BufferedMLDataSet buffer = new BufferedMLDataSet(this.EGB_FILENAME);
		buffer.beginLoad(2, 1);
		for (int i = 0; i < XOR.XOR_INPUT.length; i++) {
			buffer.add(new BasicMLDataPair(new BasicMLData(XOR.XOR_INPUT[i]), new BasicMLData(XOR.XOR_IDEAL[i])));
		}
		buffer.endLoad();

		NEATPopulation pop = new NEATPopulation(2, 1, 1000);
		pop.setInitialConnectionDensity(1.0);// not required, but speeds
												// training
		pop.reset();
