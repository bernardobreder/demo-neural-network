...
public class TestCompetitive extends TestCase {
	...
	public static double SOM_INPUT[][] = { { 0.0, 0.0, 1.0, 1.0 }, { 1.0, 1.0, 0.0, 0.0 } };
	...
	public void testSOM() {
		...
		final MLDataSet training = new BasicMLDataSet(TestCompetitive.SOM_INPUT, null);

		// Create the neural network.
		SOM network = new SOM(4, 2);
		network.setWeights(new Matrix(MATRIX_ARRAY));

		final BasicTrainSOM train = new BasicTrainSOM(network, 0.4, training, new NeighborhoodSingle());
		...
		final MLData data1 = new BasicMLData(TestCompetitive.SOM_INPUT[0]);
		final MLData data2 = new BasicMLData(TestCompetitive.SOM_INPUT[1]);
