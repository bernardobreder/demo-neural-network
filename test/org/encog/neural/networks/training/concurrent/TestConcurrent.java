...
public class TestConcurrent extends TestCase {
	private void internalTest(boolean splitCores) {
		...
		// create a RPROP job
		TrainingJob job1 = new RPROPJob(XOR.createUnTrainedXOR(), XOR.createXORDataSet(), true);
		job1.getStrategies().add(new EndMaxErrorStrategy(0.01));
		ctm.addTrainingJob(job1);

		// create a BPROP job
		TrainingJob job2 = new BPROPJob(XOR.createUnTrainedXOR(), XOR.createXORDataSet(), true, 0.07, 0.07);
