...
public class TestPersistHMM extends TestCase {
	...
	public void testDiscPersistEG() {
		...
		EncogDirectoryPersistence.saveObject(this.EG_FILENAME, sourceHMM);
		HiddenMarkovModel resultHMM = (HiddenMarkovModel) EncogDirectoryPersistence.loadObject(this.EG_FILENAME);

		this.validate(resultHMM, sourceHMM);
	}

	public void testDiscPersistSerial() throws IOException, ClassNotFoundException {
		HiddenMarkovModel sourceHMM = buildDiscHMM();

		SerializeObject.save(this.SERIAL_FILENAME, sourceHMM);
		HiddenMarkovModel resultHMM = (HiddenMarkovModel) SerializeObject.load(this.SERIAL_FILENAME);
