...
public class TestPersistNEAT extends TestCase {
	...
	private NEATNetwork create() {
		// simple network, 1 input, 1 bais , those two both conned into output
		// neuron, so that 2 links!
		ActivationFunction[] activationFunctions = new ActivationFunction[3];
		List<NEATLink> links = new ArrayList<>(2);
	...
	public void testPersistSerial() throws IOException, ClassNotFoundException {
		NEATNetwork network = this.create();

		SerializeObject.save(this.SERIAL_FILENAME, network);
		NEATNetwork network2 = (NEATNetwork) SerializeObject.load(this.SERIAL_FILENAME);
