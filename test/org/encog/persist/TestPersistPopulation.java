...
public class TestPersistPopulation extends TestCase {
	...
	private NEATPopulation generate() {
		...
		EvolutionaryAlgorithm train = NEATUtil.constructNEATTrainer(score, 2, 1, 10);
	...
	public void testPersistEG() {
		Population pop = this.generate();

		EncogDirectoryPersistence.saveObject((this.EG_FILENAME), pop);
		NEATPopulation pop2 = (NEATPopulation) EncogDirectoryPersistence.loadObject((this.EG_FILENAME));

		this.validate(pop2);
	}

	public void testPersistSerial() throws IOException, ClassNotFoundException {
		NEATPopulation pop = this.generate();
		this.validate(pop);

		SerializeObject.save(this.SERIAL_FILENAME, pop);
		NEATPopulation pop2 = (NEATPopulation) SerializeObject.load(this.SERIAL_FILENAME);
